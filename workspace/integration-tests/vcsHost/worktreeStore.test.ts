...
// The WorktreeStore primitive accepts any logId; these store-level tests use a single
...
describe("WorktreeStore snapshot/materialize", () => {
  let root: string;
  let gad: TestGad;
  ...
  it("subtree hashes (content store) change only for touched subtrees", async () => {
    ...
    // no DO round trip (snapshots mirror eagerly).
