...
describe("WorkspaceVcs.listFiles / revert / pushStatus", () => {
  ...
  async function seedMain(
    ...
    const pushed = await pushToMain(gad, { repoPaths: [repo], sourceHead: seedHead, actor });
  ...
  it("pushStatus does not count upstream-only drift as local ahead", async () => {
    ...
    expect((await pushToMain(gad, { repoPaths: [repoPath], sourceHead: head, actor: USER })).status).toBe(
      "pushed"
    );
