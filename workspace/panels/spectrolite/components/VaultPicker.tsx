...
Replace these starter contents with your own when you're ready.
`;
...
export function VaultPicker({ agentHandle, onSelect }: VaultPickerProps) {
  ...
  const validationError = useMemo(
    () => (newName.trim() ? validateVaultName(newName) : null),
    [newName]
  );
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (duplicateName) {
      setCreateError(`A vault named "${trimmed}" already exists`);
      return;
    }
    setCreateError(null);
    setCreating(true);
          ...
          <Flex direction="column" gap="2">
            ...
            <Flex gap="2">
              <TextField.Root
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !validationError && !duplicateName) void handleCreate();
                }}
                style={{ flex: 1 }}
                disabled={creating}
              />
            ...
            {validationError ? (
              ...
              <Text size="1" color="red">
                A vault named "{newName.trim()}" already exists
              </Text>
            ) : (
              <Text size="1" color="gray">
