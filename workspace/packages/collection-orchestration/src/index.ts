...
export function buildCollectionAgentSystemPrompt(args: {
  ...
  return [
    ...
    "```",
    "",
    "The scope is recursive and live. `scope.revision` changes with tree state; `scope.nodes`, `scope.descendants`, and `scope.leaves` contain nodes with `.handle`, `.depth`, and `.children`. Never rely on a member list copied into a prompt.",
    "",
    ...
    "- Reparent or reorder with `await node.handle.movePanel(newParentId, position)`.",
    "- Create nested collection panels in this root's orchestration context (`scope.root.handle.contextId`) so their resident conductors inherit the same prompt-free control boundary.",
    '- Example: `openPanel("about/collection", { parentId, contextId: scope.root.handle.contextId, title, focus: false, stateArgs: { title, note } })`.',
...
export function buildBrowserImportWindowTitlePrompt(sourceName: string): string {
  return [
    ...
    "Refresh the recursive subtree first. Identify the descendant `about/collection` panels whose state `origin` marks them as imported browser windows.",
    ...
    "Refresh the subtree after the batch and briefly summarize the window titles you changed.",
  ].join(" ");
}
