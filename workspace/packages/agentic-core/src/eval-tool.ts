...
\`return\` sends a value back to the agent. \`console.log\` streams in real time. \`scope\` persists across eval calls.`,
    parameters: z
      .object({
        code: z
          .string()
          .optional()
          .describe("The TypeScript/JavaScript code to execute. Provide either code or path."),
        path: z
          .string()
          .optional()
          .describe(
            "Context-relative TypeScript/TSX file to execute instead of inline code. Supports static relative imports."
          ),
        ...
        imports: z
          ...
          .optional()
          .describe(
            'On-demand package builds. Workspace packages (@workspace/*, @natstack/*) are auto-resolved and don\'t need this. Use for npm packages ("npm:<version>") or to pin a workspace package to a specific git ref.'
          ),
      })
      .strict(),
    streaming: true,
    execute: async (args: unknown, ctx: MethodExecutionContext) => {
      ...
      const code = path ? await loadEvalSource(opts.rpc, path) : typedArgs.code;
      if (!code) throw new Error("Missing code or path");
          ...
          bindings: {
            ...
            help: async (serviceName?: string) => {
              ...
              return {
                ...
                runtimeUsage:
                  'Runtime exports are not globals. Import them with static syntax, e.g. `import { contextId, fs, rpc } from "@workspace/runtime";`.',
                services,
                runtime,
                imports: {
                  description:
                    "Use the eval tool's `imports` parameter to load additional packages on-demand.",
                  usage:
                    'Workspace packages (@workspace/*, @natstack/*) are auto-resolved. For npm: imports: { "lodash": "npm:4" }. To pin a git ref: imports: { "pkg": "branch-name" }',
                  workspaceSkills: skillPackages,
                  npmPackages:
                    'Use "npm:<version>" for npm packages, e.g. "npm:latest" or "npm:^4.0.0"',
                },
              };
            },
          },
...
function formatEvalReturnValue(
  ...
  opts: {
    onOversize: () => void;
    spillText: (text: string) => Promise<EvalBlobRef | null>;
    scopeKey: string;
  }
): Promise<string> {
...
async function boundEvalText(
  ...
  if (text.length <= MAX_EVAL_TEXT_PART_CHARS) return text;
...
function summarizeArray(value: unknown[]): string {
  ...
  if (objectKeys.length > 0) {
    lines.push(
      `Common keys: ${objectKeys.map(({ key, count }) => `${key} (${count})`).join(", ")}.`
    );
  }
...
function summarizeDiagnosticArray(value: unknown[]): string | null {
  const diagnostics = value.filter((item): item is Record<string, unknown> =>
    isDiagnosticLike(item)
  );
  ...
  lines.push(
    `By severity: ${formatCounts(
      mostCommon(
        diagnostics.map((d) => stringField(d, "severity") ?? "unknown"),
        8
      )
    )}.`
  );
  ...
  const codes = mostCommon(
    diagnostics.map((d) => stringField(d, "code")).filter((code): code is string => Boolean(code)),
    8
  );
  if (codes.length > 0) lines.push(`Top codes: ${formatCounts(codes)}.`);

  const files = mostCommon(
    diagnostics.map((d) => stringField(d, "file")).filter((file): file is string => Boolean(file)),
    8
  );
  ...
  lines.push(
    ...diagnostics.slice(0, 5).map((diagnostic, index) => {
      ...
      const location = file
        ...
        : "(no file)";
      const message = truncateSingleLine(
        stringField(diagnostic, "message") ?? compactJson(diagnostic, 180),
        220
      );
...
function isDiagnosticLike(value: unknown): value is Record<string, unknown> {
  ...
  return (
    ...
    ("severity" in record || "code" in record || "file" in record || "line" in record)
  );
}
...
function withSandboxErrorHint(error: string): string {
  ...
  if (!missingRuntimeBinding) return error;
  ...
  if (!runtimeExports.has(name)) return error;
