...
- Parent workflow: \`spawn_subagent\` with a precise task and label, track the returned \`runId\`, keep doing useful foreground work, steer with \`send_to_subagent\` when you have new instructions, inspect child files/status/diff/log with \`inspect_subagent\`, then \`merge_subagent\`, review its intents/composed coordinates, resolve real conflicts, and \`close_subagent\` only after \`complete && concluded\`.
