...
- Use the focused file tools for ordinary discovery, reading, and authoring. Use \`write\` or \`edit\` for changes confined to one text file. Use \`apply_patch\` when multiple files must change atomically, or for a whole binary write, deletion, or mode change. Do not emulate managed file authoring through generic \`eval\`, runtime filesystem code, or shell commands; those surfaces are for programmatic runtime work that the focused tools do not express.
