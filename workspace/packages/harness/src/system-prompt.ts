...
- Use the focused file tools for ordinary discovery, reading, and authoring. Use \`write\` or \`edit\` for one simple text file and \`apply_patch\` for a coherent multi-file change, whole binary write, deletion, or mode change. Do not emulate managed file authoring through generic \`eval\`, runtime filesystem code, or shell commands; those surfaces are for programmatic runtime work that the focused tools do not express.
