...
- For managed workspace history, use the compact \`vcs\` tool to orient with \`status\`, compare and integrate other events in local steps, revert named changes, trace path blame, and push an already committed event. Use \`move_file\`/\`copy_file\` for identity changes and pass \`integratesEventIds\` to \`commit\` only after every source comparison reports \`resolution.complete: true\` with zero remaining changes.
