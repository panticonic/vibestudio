...
export interface GadClient {
  ...
  ensureBranch(input: GadEnsureBranchInput): Promise<{ id: string }>;
  recordSession(input: GadRecordSessionInput): Promise<{ id: string }>;
  endSession(sessionId: string, endedAt?: string | null): Promise<void>;
  recordTurn(input: GadRecordTurnInput): Promise<{ id: number; turnIndex: number }>;
  beginToolCall(input: GadBeginToolCallInput): Promise<{ id: number }>;
  completeToolCall(toolCallId: number, resultSummary?: string | null, completedAt?: string | null): Promise<void>;
  recordRead(input: GadRecordReadInput): Promise<{ id: number }>;
  recordMutation(input: GadRecordMutationInput): Promise<{ id: number }>;
  ...
  forkBranch(input: GadEnsureBranchInput & { parentBranchId: string }): Promise<{ id: string }>;
  createBranchSnapshot(input: GadCreateBranchSnapshotInput): Promise<{ id: number }>;
  ...
  addChunkMention(input: { chunkHash: string; attribution?: string | null; sourceSessionId?: string | null; sourceTurnId?: number | null }): Promise<void>;
  relateChunk(chunkHash: string, targetType: string, targetHash: string): Promise<void>;
  listChunks(input?: { attribution?: string | null; since?: string | null }): Promise<GadJsonRecord[]>;
  ...
  walkDependencies(chunkHash: string, input?: { maxDepth?: number; targetTypes?: string[] }): Promise<{ nodes: GadJsonRecord[]; edges: Array<{ from: string; to: string; relationType: string }> }>;
  upsertChunkEmbedding(input: GadVectorInput & { chunkHash: string }): Promise<void>;
  upsertTurnEmbedding(input: GadVectorInput & { turnId: number }): Promise<void>;
  ...
  indexFileVersion(input: { path: string; contentHash: string; content: string }): Promise<{ structures: GadJsonRecord[]; chunk: GadJsonRecord }>;
  indexSession(sessionId: string): Promise<{ turnsIndexed: number; fileVersionsIndexed: number }>;
  ...
  listBlobReferences(input?: { includeUnreferenced?: boolean }): Promise<GadJsonRecord[]>;
...
export function createGadClient(rpc: RpcCaller): GadClient {
  return {
    ...
    completeToolCall: (toolCallId, resultSummary, completedAt) =>
      rpc.call("main", "gad.completeToolCall", toolCallId, resultSummary, completedAt),
