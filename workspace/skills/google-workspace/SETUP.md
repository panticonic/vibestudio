...
open each link inside Vibestudio or in the system browser.

This setup is modeled on gogcli's Google quick start: create/select one Google
Cloud project, enable the Workspace bundle Vibestudio requests up front, configure
...
the account. Vibestudio asks Google for broad durable Workspace access once, then
...
6. Run `connectGoogle()` to launch the host-owned PKCE flow. The helper asks
   Google for offline access and tells Vibestudio to persist the returned refresh
   token.
...
const requiredSteps = [
  ...
  },
  {
    ...
    note: "Choose Application type: Desktop app. You will enter its client_id and client_secret in Vibestudio's trusted approval UI.",
  },
];
...
- **Internal** opens a Vibestudio browser panel. Prefer this when the user may
...
With it, Gmail pushes new-mail notifications to the vibestudio server and syncs
arrive in seconds; polling drops to a 30-minute safety net.

Requirements: a GCP project, a Pub/Sub topic, and vibestudio server reachable by Google Pub/Sub via the callback relay (see docs/webrtc-rpc-transport.md §7).
...
1. Create the topic and grant Gmail publish rights:
   ...
   ```
2. Create one generic Vibestudio webhook subscription for Google Cloud Pub/Sub
...
3. Create a Google Pub/Sub push subscription pointing at that generic webhook
   URL:
   ```bash
   gcloud pubsub subscriptions create gmail-push-vibestudio \
     --topic gmail-push \
