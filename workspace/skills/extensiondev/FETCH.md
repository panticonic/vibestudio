...
A common pattern is to expose both: a typed RPC surface for in-app callers and a thin fetch handler that delegates to the same internal helpers. See `extensions/browser-data/` for an example of the dual surface.
