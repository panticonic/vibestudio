...
The directory layout matches `panels/` and `workers/`. Each extension is a workspace unit; the build graph discovers it via the `vibestudio.extension` block in `package.json`.
