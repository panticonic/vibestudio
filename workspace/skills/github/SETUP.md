...
permissions. The default Vibestudio-friendly path is broad upstream access:
...
`requestGitHubTokenCredential()`, and verify it. Vibestudio then stages access
...
2. Set a token name such as `Vibestudio`.
...
- API and Git: both of the above.
  ...
  commits `meta/vibestudio.yml`, propagates the remote into workspace contexts,
...
- **Internal** opens a Vibestudio browser panel with
      ...
      <Flex align="start" justify="between" gap="3" wrap="wrap">
        <Box>
          ...
          <Text size="2" color="gray">
            Choose a token style and access level, then save the generated token in Vibestudio's
            trusted prompt.
          </Text>
        </Box>
