...
NatStack mobile has two layers:
...
- `natstack://connect` clean-install pairing
...
native host requests. The current native host requests `NatStack`, so the active
...
2. User opens a `natstack://connect?url=...&code=...` link on the phone.
