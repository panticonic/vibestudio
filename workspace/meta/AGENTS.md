...
- For `eval`, `inline_ui`, `load_action_bar`, and `feedback_custom`, prefer a context-relative `path` over large inline code when the implementation spans files. File-loaded sources support static relative imports.
