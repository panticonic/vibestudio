Workspace-local operating guide for Vibez1 agents. This section focuses on workspace-specific paths, APIs, and diagnostics.
...
Some skills also export code you can use in eval. Workspace packages (`@workspace-skills/*`, `@workspace/*`, `@vibez1/*`) are **auto-resolved** — just write the `import` and they're built on first use:
...
- **subagents** — delegating work to child agents, supervising task channels, reading/inspecting child work, and merging or picking results
- **onboarding** — first-time setup, workspace configuration, Vibez1 overview
...
From a terminal, the same data is available via the external-agent CLI: `vibez1 agent diag UNIT` and `vibez1 agent logs UNIT [--level error]`. `agent diag` is for already-running units; for whether a change *builds*, the `vcs push` report is the source of truth.
