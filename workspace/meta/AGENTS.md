...
- **onboarding** — first-time setup, workspace configuration, Vibestudio overview
...
From a terminal, the same data is available via the external-agent CLI: `vibestudio agent diag UNIT` and `vibestudio agent logs UNIT [--level error]`. `agent diag` is for already-running units; for whether a change *builds*, the `vcs push` report is the source of truth.
