 ...
 * events through the native app bridge.
 *
 * Redirect policy belongs to the caller that starts the OAuth flow. Mobile
 * only preserves the requested strategy, with a convenience path for
 * server-coordinated client-forwarded universal-link callbacks.
 */
import type {
...
export function buildMobileOAuthRedirectUri(
  ...
  const origin = callbackOrigin.endsWith("/") ? callbackOrigin.slice(0, -1) : callbackOrigin;
...
export async function connectMobileOAuthCredential(
  ...
  if (wantsClientForwarded) {
    let callbackUri = request.redirectUri;
    if (!callbackUri) {
      const providerId = request.providerId;
      if (!providerId) {
        throw new Error("client-forwarded mobile OAuth requires providerId or redirectUri");
      }
      callbackUri = buildMobileOAuthRedirectUri(providerId, request.callbackOrigin);
    }
    redirect = {
      ...(request.redirect ?? {}),
...
export async function waitForMobileOAuthCode(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    void openExternalUrl(authUrl.toString()).catch((err: unknown) => {
      dropPendingFlow(expectedState);
      clearTimeout(timer);
