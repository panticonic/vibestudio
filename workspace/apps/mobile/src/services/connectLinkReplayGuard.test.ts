...
describe("connectLinkReplayGuard", () => {
  ...
  it("suppresses a consumed connect link throughout its replay TTL", async () => {
    const rawUrl =
      "vibestudio://connect?url=https%3A%2F%2Fhost.tailnet.ts.net&code=abc123abc123abc123";
  ...
  it("does not suppress a different connect link", async () => {
    storage.getItem.mockResolvedValueOnce(
      JSON.stringify({
        url: "vibestudio://connect?url=https%3A%2F%2Fold.example&code=abc123abc123abc123",
        consumedAt: 1_000,
      })
    );
    ...
    await expect(
      consumeConnectLinkReplay(
        "vibestudio://connect?url=https%3A%2F%2Fnew.example&code=def123def123def123",
        2_000
      )
    ).resolves.toBe(false);
    expect(storage.removeItem).not.toHaveBeenCalled();
