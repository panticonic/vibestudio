 ...
 * universal-link callback, then exchanges the authorization code with the
 * server over the existing mobile RPC transport.
 */
...
export async function launchConsentFlow(params: LaunchConsentFlowParams): Promise<ConsentCallbackParams | null> {
    ...
    return new Promise<ConsentCallbackParams | null>((resolve, reject) => {
        ...
        void openExternalUrl(authorizeUrl.toString()).catch((error: unknown) => {
            pendingFlows.delete(state);
            clearTimeout(timer);
