...
@interface Vibez1MobileHost : NSObject <RCTBridgeModule>
@end

@implementation Vibez1MobileHost

RCT_EXPORT_MODULE();

static NSString *const Vibez1KeychainService = @"com.vibez1.mobile.host";
static NSString *const Vibez1CredentialAccount = @"mobile-refresh";
...
static NSString *const Vibez1ActiveBundleSource = @"activeBundle.source";
static NSString *const Vibez1WorkspaceAppCallerPrefix = @"app:apps/";
static NSTimeInterval const Vibez1JsonPostTimeout = 120;
static NSTimeInterval const Vibez1BundleDownloadTimeout = 30;
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  @try {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      RCTTriggerReloadCommandListeners(@"Vibez1 mobile host reset");
    });
  } @catch (NSException *exception) {
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
    @try {
      NSDictionary *credential = [self loadCredential];
      if (credential == nil) {
        [NSException raise:@"Vibez1NoCredentials" format:@"No mobile credentials are stored"];
      }
      NSString *hubUrl = [self optionalString:credential key:@"hubUrl"];
      if (hubUrl.length == 0) {
        [NSException raise:@"Vibez1NoHubURL" format:@"Stored credential is missing a hub URL"];
      }
  ...
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
    @try {
      ...
      if (![bootstrap isKindOfClass:[NSDictionary class]]) {
        [NSException raise:@"Vibez1BundleBootstrapInvalid" format:@"Mobile app bootstrap payload is invalid"];
      }
      NSString *rnHostAbi = [self requiredString:bootstrap key:@"rnHostAbi"];
      if (![rnHostAbi isEqualToString:expectedRnHostAbi]) {
        [NSException raise:@"Vibez1RnHostAbiMismatch" format:@"React Native host ABI mismatch: expected %@, got %@", expectedRnHostAbi, rnHostAbi];
      }
      ...
      if ([source isKindOfClass:[NSString class]] && source.length > 0) {
        [[NSUserDefaults standardUserDefaults] setObject:source forKey:Vibez1ActiveBundleSource];
      } else {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:Vibez1ActiveBundleSource];
      }
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  @try {
    ...
    if (bundleData == nil) {
      [NSException raise:@"Vibez1BundleActivationInvalid" format:@"Prepared React Native bundle could not be read"];
    }
    [self verifySha256Integrity:integrity data:bundleData];
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    BOOL changed =
      ![[defaults stringForKey:Vibez1ActiveBundleLocalPath] isEqualToString:canonicalPath] ||
      ![[defaults stringForKey:Vibez1ActiveBundleBuildKey] isEqualToString:buildKey] ||
      ![[defaults stringForKey:Vibez1ActiveBundleIntegrity] isEqualToString:integrity];
    [defaults setObject:canonicalPath forKey:Vibez1ActiveBundleLocalPath];
    [defaults setObject:buildKey forKey:Vibez1ActiveBundleBuildKey];
    [defaults setObject:integrity forKey:Vibez1ActiveBundleIntegrity];
    [defaults synchronize];
    resolve(@{ @"activated": @(changed) });
    if (changed) {
      dispatch_async(dispatch_get_main_queue(), ^{
        RCTReloadCommandSetBundleURL([NSURL fileURLWithPath:canonicalPath]);
        RCTTriggerReloadCommandListeners(@"Vibez1 workspace app bundle activated");
      });
    }
  } @catch (NSException *exception) {
...
- (NSDictionary *)shellGrantFromResponse:(NSDictionary *)response credential:(NSDictionary *)credential
{
  ...
  if (![self isMobileShellCaller:callerId deviceId:credential[@"deviceId"]]) {
    [NSException raise:@"Vibez1ShellGrantInvalid" format:@"Mobile shell grant response did not include this device's shell principal"];
  }
  if (shellToken.length == 0) {
    [NSException raise:@"Vibez1ShellGrantInvalid" format:@"Mobile shell grant response did not include a shell token"];
  }
  if (responseDeviceId.length > 0 && ![responseDeviceId isEqualToString:credential[@"deviceId"]]) {
    [NSException raise:@"Vibez1ShellGrantInvalid" format:@"Mobile shell grant response device did not match the stored credential"];
  }
  NSMutableDictionary *result = [@{
    @"serverUrl": credential[@"serverUrl"],
    @"deviceId": credential[@"deviceId"],
...
- (NSDictionary *)issueAppGrantForCredential:(NSDictionary *)credential source:(NSString *)source
{
  ...
  if (![self isWorkspaceMobileAppCaller:callerId deviceId:credential[@"deviceId"]]) {
    [NSException raise:@"Vibez1AppGrantInvalid" format:@"Mobile app grant response did not include a workspace mobile app principal"];
  }
  if (connectionGrant.length == 0) {
    [NSException raise:@"Vibez1AppGrantInvalid" format:@"Mobile app grant response did not include a connection grant"];
  }
  if (responseDeviceId.length > 0 && ![responseDeviceId isEqualToString:credential[@"deviceId"]]) {
    [NSException raise:@"Vibez1AppGrantInvalid" format:@"Mobile app grant response device did not match the stored credential"];
  }
  NSMutableDictionary *result = [@{
    @"serverUrl": credential[@"serverUrl"],
    @"deviceId": credential[@"deviceId"],
...
{
  NSString *source = [[NSUserDefaults standardUserDefaults] stringForKey:Vibez1ActiveBundleSource];
...
- (void)clearActiveBundle
{
  ...
  [defaults removeObjectForKey:Vibez1ActiveBundleSource];
  [defaults synchronize];
}

- (BOOL)isWorkspaceMobileAppCaller:(NSString *)callerId deviceId:(NSString *)deviceId
{
  return [callerId hasPrefix:Vibez1WorkspaceAppCallerPrefix] &&
    deviceId.length > 0 &&
...
- (NSDictionary *)postJson:(NSString *)serverUrl path:(NSString *)path body:(NSDictionary *)body
{
  ...
  request.timeoutInterval = Vibez1JsonPostTimeout;
  ...
  if (requestError != nil) {
    [NSException raise:@"Vibez1AuthRequestFailed" format:@"%@", requestError.localizedDescription];
  }
  NSDictionary *json = @{};
  ...
  if (httpResponse.statusCode < 200 || httpResponse.statusCode >= 300) {
    ...
    [NSException raise:@"Vibez1AuthRequestFailed" format:@"%@", message];
  }
  return json;
}
...
- (NSData *)getData:(NSString *)urlString
{
  ...
  request.timeoutInterval = Vibez1BundleDownloadTimeout;
  ...
  if (requestError != nil) {
    [NSException raise:@"Vibez1BundleRequestFailed" format:@"%@", requestError.localizedDescription];
  }
  if (httpResponse.statusCode < 200 || httpResponse.statusCode >= 300) {
    NSString *message = [[NSString alloc] initWithData:responseData encoding:NSUTF8StringEncoding] ?: @"";
    [NSException raise:@"Vibez1BundleRequestFailed" format:@"Bundle artifact request failed (%ld): %@", (long)httpResponse.statusCode, message];
  }
...
- (void)assertArtifactURL:(NSString *)artifactUrl sameOriginAsServer:(NSString *)serverUrl
{
  ...
  if (!sameOrigin) {
    [NSException raise:@"Vibez1BundleBootstrapInvalid" format:@"React Native bundle artifact URL is outside the paired server origin"];
  }
}
...
- (NSDictionary *)selectArtifact:(NSDictionary *)bootstrap platform:(NSString *)platform
{
  ...
  if (![artifacts isKindOfClass:[NSArray class]]) {
    [NSException raise:@"Vibez1BundleBootstrapInvalid" format:@"Mobile app bootstrap artifacts are invalid"];
  }
  ...
  [NSException raise:@"Vibez1BundleBootstrapInvalid" format:@"No primary React Native bundle artifact is available for %@", platform];
  return @{};
}
...
- (void)verifySha256Integrity:(NSString *)integrity data:(NSData *)data
{
  ...
  if ([regex numberOfMatchesInString:expected options:0 range:NSMakeRange(0, expected.length)] != 1) {
    [NSException raise:@"Vibez1BundleIntegrityUnsupported" format:@"Unsupported React Native bundle integrity: %@", integrity];
  }
  ...
  if ([actual caseInsensitiveCompare:expected] != NSOrderedSame) {
    [NSException raise:@"Vibez1BundleIntegrityMismatch" format:@"React Native bundle integrity mismatch"];
  }
}
...
- (NSString *)writeBundleData:(NSData *)data buildKey:(NSString *)buildKey artifactPath:(NSString *)artifactPath
{
  ...
  NSURL *dirURL = [[cacheURL URLByAppendingPathComponent:@"vibez1-rn" isDirectory:YES] URLByAppendingPathComponent:safeBuildKey isDirectory:YES];
  ...
  if (![data writeToURL:fileURL atomically:YES]) {
    [NSException raise:@"Vibez1BundleCacheWriteFailed" format:@"Could not write prepared React Native bundle"];
  }
  return fileURL.path;
}
...
- (NSString *)validatedPreparedBundlePath:(NSString *)localPath
{
  ...
  NSURL *cacheURL = [[NSFileManager.defaultManager URLsForDirectory:NSCachesDirectory inDomains:NSUserDomainMask] firstObject];
  NSString *bundleRoot = [[[cacheURL URLByAppendingPathComponent:@"vibez1-rn" isDirectory:YES] path] stringByResolvingSymlinksInPath];
  ...
  if (!isUnderRoot || ![NSFileManager.defaultManager fileExistsAtPath:canonicalPath isDirectory:&isDirectory] || isDirectory) {
    [NSException raise:@"Vibez1BundleActivationInvalid" format:@"Prepared React Native bundle is outside the app cache"];
  }
...
- (void)saveCredential:(NSDictionary *)credential
{
  ...
  NSDictionary *query = @{
    ...
    (__bridge id)kSecAttrService: Vibez1KeychainService,
    (__bridge id)kSecAttrAccount: Vibez1CredentialAccount,
    (__bridge id)kSecAttrAccessible: (__bridge id)kSecAttrAccessibleWhenUnlockedThisDeviceOnly,
    (__bridge id)kSecValueData: data,
  };
  OSStatus status = SecItemAdd((__bridge CFDictionaryRef)query, NULL);
  if (status != errSecSuccess) {
    [NSException raise:@"Vibez1KeychainSaveFailed" format:@"Could not store mobile credentials (%d)", (int)status];
  }
}
...
- (NSDictionary *)loadCredential
{
  ...
  if (status != errSecSuccess) {
    [NSException raise:@"Vibez1KeychainLoadFailed" format:@"Could not load mobile credentials (%d)", (int)status];
  }
  ...
  if (![credential isKindOfClass:[NSDictionary class]]) {
    [NSException raise:@"Vibez1CredentialRepair" format:@"Stored credential payload is invalid"];
  }
...
- (NSString *)normalizeServerUrl:(NSString *)serverUrl
{
  ...
  if (![scheme isEqualToString:@"http"] && ![scheme isEqualToString:@"https"]) {
    [NSException raise:@"Vibez1InvalidServerURL" format:@"Pairing server URL must use http or https"];
  }
  if (components.host.length == 0 || components.user.length > 0 || components.password.length > 0) {
    [NSException raise:@"Vibez1InvalidServerURL" format:@"Pairing server URL must be an origin"];
  }
  if (
    ...
    [NSException raise:@"Vibez1InvalidServerURL" format:@"Pairing server URL must not include a path, query, or fragment"];
  }
...
- (NSString *)normalizeWorkspaceServerUrl:(NSString *)serverUrl
{
  ...
  NSString *scheme = components.scheme.lowercaseString ?: @"";
  if (![scheme isEqualToString:@"http"] && ![scheme isEqualToString:@"https"]) {
    [NSException raise:@"Vibez1InvalidServerURL" format:@"Workspace server URL must use http or https"];
  }
  if (components.host.length == 0 || components.user.length > 0 || components.password.length > 0) {
    [NSException raise:@"Vibez1InvalidServerURL" format:@"Workspace server URL must include a host"];
  }
  if (components.query.length > 0 || components.fragment.length > 0) {
    [NSException raise:@"Vibez1InvalidServerURL" format:@"Workspace server URL must not include a query or fragment"];
  }
  components.scheme = scheme;
...
- (NSString *)requiredString:(NSDictionary *)dictionary key:(NSString *)key
{
  ...
  if (![value isKindOfClass:[NSString class]] || [value length] == 0) {
    [NSException raise:@"Vibez1MissingField" format:@"Missing credential field: %@", key];
  }
  return value;
}
...
- (NSArray *)requiredStringArray:(NSDictionary *)dictionary key:(NSString *)key
{
  ...
  if (![value isKindOfClass:[NSArray class]]) {
    [NSException raise:@"Vibez1MissingField" format:@"Missing string array field: %@", key];
  }
  for (id item in (NSArray *)value) {
    if (![item isKindOfClass:[NSString class]] || [item length] == 0) {
      [NSException raise:@"Vibez1InvalidField" format:@"Invalid string array field: %@", key];
    }
  }
  return value;
}
