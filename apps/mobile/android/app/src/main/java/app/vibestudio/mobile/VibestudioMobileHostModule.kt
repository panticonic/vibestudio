...
class VibestudioMobileHostModule(
    ...
    fun pairServer(serverUrl: String, code: String, promise: Promise) {
        thread(start = true, isDaemon = true, name = "VibestudioPairing") {
            try {
                ...
                Log.i(TAG, "[VibestudioMobileSmoke] phase=native-pairing-complete")
                promise.resolve(Arguments.createMap().apply {
    ...
    fun listWorkspaces(promise: Promise) {
        thread(start = true, isDaemon = true, name = "VibestudioWorkspaceList") {
            try {
                val credential = loadCredential()
    ...
    fun selectWorkspace(name: String, source: String?, promise: Promise) {
        thread(start = true, isDaemon = true, name = "VibestudioWorkspaceSelect") {
            try {
                val credential = loadCredential()
                    ?: throw IllegalStateException("No mobile credentials are stored")
                val hubUrl = credential.hubUrl
    ...
    fun issueConnectionGrant(promise: Promise) {
        thread(start = true, isDaemon = true, name = "VibestudioConnectionGrant") {
            try {
                val credential = loadCredential()
                    ?: throw IllegalStateException("No mobile credentials are stored")
    ...
    fun prepareAppBundle(expectedRnHostAbi: String, platform: String, source: String?, promise: Promise) {
        thread(start = true, isDaemon = true, name = "VibestudioPrepareBundle") {
            try {
                val credential = loadCredential()
                    ?: throw IllegalStateException("No mobile credentials are stored")
                val body = JSONObject()
                ...
                Log.i(TAG, "[VibestudioMobileSmoke] phase=native-bundle-bootstrap-fetched")
                ...
                val activeBundlePath = VibestudioBundleStore.activeBundlePathIfMatches(
                ...
                if (activeBundlePath != null) {
                    ...
                    Log.i(TAG, "[VibestudioMobileSmoke] phase=native-bundle-cache-hit")
                    Log.i(TAG, "[VibestudioMobileSmoke] phase=native-bundle-prepared cached=true")
                ...
                updateActiveAppSource(source)
                Log.i(TAG, "[VibestudioMobileSmoke] phase=native-bundle-prepared")
                promise.resolve(preparedBundleMap(
                    bootstrap,
                    buildKey,
                    rnHostAbi,
    ...
    private fun saveCredential(credential: Credential) {
        ...
        Log.i(
            TAG,
            "[VibestudioMobileSmoke] phase=native-save-credential pid=${android.os.Process.myPid()} committed=$committed workspaceId='${credential.workspaceId}'"
        )
    }
    ...
    private fun loadCredential(): Credential? {
        ...
        Log.i(
            TAG,
            "[VibestudioMobileSmoke] phase=native-load-credential pid=${android.os.Process.myPid()} present=${encrypted != null}"
        )
    ...
    private companion object {
        ...
        const val KEY_ALIAS = "vibestudio-mobile-refresh"
