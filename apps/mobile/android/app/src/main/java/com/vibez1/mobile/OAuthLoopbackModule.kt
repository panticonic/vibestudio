...
class OAuthLoopbackModule(
    ...
    fun start(options: ReadableMap, promise: Promise) {
        ...
        try {
            ...
            promise.reject("keepalive_failed", "Could not keep Vibez1 active for browser sign-in", error)
            return
        }
        ...
        for (bound in sockets) {
            next.threads += thread(
                start = true,
                isDaemon = true,
                name = "Vibez1OAuthLoopback-${bound.label}",
            ) {
    ...
    private fun handleClient(active: Listener, client: Socket, bound: BoundSocket): Boolean {
        client.use { socket ->
            ...
            if (error != null) {
                respond(socket.getOutputStream(), 400, "The provider denied the connection. Return to Vibez1.")
                resolveWait(active, rawUrl, code, state, error)
                finish(active)
                return true
            }
            if (code.isNullOrBlank()) {
                respond(socket.getOutputStream(), 400, "Missing authorization code. Return to Vibez1.")
                rejectWait(active, "missing_code", "OAuth callback did not include an authorization code")
                finish(active)
                return true
            }

            respond(socket.getOutputStream(), 200, "Connection complete. You can return to Vibez1.")
