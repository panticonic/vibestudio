...
class Vibez1MobileHostModule(
    reactContext: ReactApplicationContext,
) : ReactContextBaseJavaModule(reactContext) {
    private val prefs: SharedPreferences =
        reactContext.getSharedPreferences("vibez1-mobile-host", Context.MODE_PRIVATE)
    ...
    override fun getName(): String = "Vibez1MobileHost"

    override fun getConstants(): MutableMap<String, Any> = hashMapOf(
        "firebaseConfigured" to BuildConfig.VIBEZ1_HAS_FIREBASE
    )
    ...
    fun resetToNativeBootstrap(promise: Promise) {
        try {
            clearStoredCredentials()
            Vibez1BundleStore.clearActive(reactApplicationContext)
            promise.resolve(Arguments.createMap().apply {
                putBoolean("reloading", true)
            })
            Handler(Looper.getMainLooper()).post {
                try {
                    Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bootstrap-reset")
                    reloadReactNative()
                } catch (error: Exception) {
    ...
    fun pairServer(serverUrl: String, code: String, promise: Promise) {
        thread(start = true, isDaemon = true, name = "Vibez1Pairing") {
            try {
                ...
                Log.i(TAG, "[Vibez1MobileSmoke] phase=native-pairing-complete")
                promise.resolve(Arguments.createMap().apply {
    ...
    fun listWorkspaces(promise: Promise) {
        thread(start = true, isDaemon = true, name = "Vibez1WorkspaceList") {
            try {
                val credential = loadCredential()
    ...
    fun selectWorkspace(name: String, source: String?, promise: Promise) {
        thread(start = true, isDaemon = true, name = "Vibez1WorkspaceSelect") {
            try {
                val credential = loadCredential()
                    ?: throw IllegalStateException("No mobile credentials are stored")
                val hubUrl = credential.hubUrl
    ...
    fun issueConnectionGrant(promise: Promise) {
        thread(start = true, isDaemon = true, name = "Vibez1ConnectionGrant") {
            try {
                val credential = loadCredential()
                    ?: throw IllegalStateException("No mobile credentials are stored")
    ...
    fun prepareAppBundle(expectedRnHostAbi: String, platform: String, source: String?, promise: Promise) {
        thread(start = true, isDaemon = true, name = "Vibez1PrepareBundle") {
            try {
                val credential = loadCredential()
                    ?: throw IllegalStateException("No mobile credentials are stored")
                val body = JSONObject()
                ...
                Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bundle-bootstrap-fetched")
                ...
                val activeBundlePath = Vibez1BundleStore.activeBundlePathIfMatches(
                ...
                if (activeBundlePath != null) {
                    ...
                    Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bundle-cache-hit")
                    Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bundle-prepared cached=true")
                ...
                updateActiveAppSource(source)
                Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bundle-prepared")
                promise.resolve(preparedBundleMap(
                    bootstrap,
                    buildKey,
                    rnHostAbi,
    ...
    fun appendBundleChunk(
        ...
        try {
            if (reset) {
                ...
                val dir = File(reactApplicationContext.cacheDir, "vibez1-rn/$safeBuildKey")
                dir.mkdirs()
    ...
    fun finalizeBundleWrite(integrity: String, gzip: Boolean, promise: Promise) {
        try {
            ...
            Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bundle-prepared-from-bytes")
            promise.resolve(Arguments.createMap().apply {
    ...
    fun activatePreparedAppBundle(localPath: String, buildKey: String, integrity: String, promise: Promise) {
        try {
            val changed = Vibez1BundleStore.activate(reactApplicationContext, localPath, buildKey, integrity)
            Log.i(TAG, "[Vibez1MobileSmoke] phase=native-bundle-activated changed=$changed")
            promise.resolve(Arguments.createMap().apply {
            ...
            if (changed) {
                Handler(Looper.getMainLooper()).post {
                    try {
                        Log.i(TAG, "[Vibez1MobileSmoke] phase=native-rn-reload-requested")
                        reloadReactNative()
                    } catch (error: Exception) {
    ...
    private fun clearStoredCredentials() {
        Log.w(
            TAG,
            "[Vibez1MobileSmoke] phase=native-clear-credentials pid=${android.os.Process.myPid()}",
    ...
    private fun reloadReactNative() {
        ...
        if (BuildConfig.IS_NEW_ARCHITECTURE_ENABLED) {
            (app.reactHost ?: throw IllegalStateException("ReactHost is unavailable"))
                .reload("Vibez1 workspace app bundle activated")
        } else {
    ...
    private fun saveCredential(credential: Credential) {
        ...
        Log.i(
            TAG,
            "[Vibez1MobileSmoke] phase=native-save-credential pid=${android.os.Process.myPid()} committed=$committed workspaceId='${credential.workspaceId}'"
        )
    }
    ...
    private fun loadCredential(): Credential? {
        ...
        Log.i(
            TAG,
            "[Vibez1MobileSmoke] phase=native-load-credential pid=${android.os.Process.myPid()} present=${encrypted != null}"
        )
    ...
    private companion object {
        ...
        const val TAG = "Vibez1MobileHost"
        const val KEY_ALIAS = "vibez1-mobile-refresh"
