...
class MainActivity : ReactActivity() {
     ...
     */
    override fun getMainComponentName(): String = "Vibez1"
    ...
    private fun clearActiveBundleForConnectIntent(intent: Intent?): Boolean {
        if (!isVibez1ConnectIntent(intent)) return false
        val cleared = Vibez1BundleStore.clearActive(this)
        if (cleared) {
            Log.i(TAG, "[Vibez1MobileSmoke] phase=native-connect-intent-reset")
        }
        return cleared
    }
    ...
    private fun isVibez1ConnectIntent(intent: Intent?): Boolean {
    ...
    private companion object {
        const val TAG = "Vibez1MainActivity"
    }
}
