...
describe("webhook relay Worker — routing", () => {
  ...
  it("serves the panel-location trampoline without seeing the private fragment", async () => {
    const { env, stub } = makeEnv();
    const resp = await worker.fetch(
      new Request("https://vibestudio.app/panel#source=about%2Fserver-logs"),
      env
    );
    expect(resp.status).toBe(200);
    const html = await resp.text();
    expect(html).toContain("vibestudio://panel?");
    expect(html).toContain("location.hash");
    expect(html).not.toContain("about%2Fserver-logs");
    expect(stub.fetch).not.toHaveBeenCalled();
  });
  ...
  it("does not expose legacy provider-shaped relay paths", async () => {
    const { env, stub } = makeEnv();
    for (const url of [
      "https://vibestudio.app/calendar/lease-1",
      "https://vibestudio.app/provider/google",
    ]) {
