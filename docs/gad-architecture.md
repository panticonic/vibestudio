...
- The **gad DO** owns provenance and history: logs/commits, edit-ops, working-edit state, merge bookkeeping, and state transitions. Since P5b it also owns the merge SEMANTICS: three-way merges are computed inside the DO (`computeMerge`, backed by the userland `@workspace/vcs-engine` — vendored diff3 + MergeEngine), reading blob bytes and mirrored trees through the host `blobstore.*` RPC; the host only orchestrates the returned file set. Since P5c it ALSO owns the EDIT/COMMIT/REVERT composition (`applyEditOps`/`commitWorking`/`revertWorking`/`resolveWorkingState`, backed by the vcs-engine EditEngine): compose-base resolution (ctx head → pinned slice → protected `main` via the host `refs.*` bridge), op application with provenance hunks, conflict-marker refusal, commit re-keying, and content-store mirroring of every state it composes (`blobstore.putTree`). The host projects the returned states to disk through the `DiskProjector` follower and emits build/reactive events — it no longer assembles edit-op rows or committed file sets. Since P5d (final eviction — `src/server/gadVcs` is deleted; the permanent host surfaces live in `src/server/vcsHost/`) it ALSO owns the STATUS semantics (`vcsStatus`/`vcsPushStatus`: the unpublished delta of a head against the protected `main`, dirty/ahead/diverged/deleted), the ctx-target MERGE ORCHESTRATION (`vcsMerge`/`vcsAbortMerge`: preconditions, ours/theirs resolution, the merge commit on clean, the parked pending merge on conflict — the host projects, acknowledges materialization via `markPendingMergeMaterialized`, and emits events; a `main`-TARGET merge stays a host ref-gated push-class advance), and the CONTEXT semantics (`vcsPinContext`/`vcsResolveContextView`/`vcsContextRepoStates`/`vcsContextStatus`/`vcsRebaseContext`/`vcsDropContext`: the durable pinned base view, the self-invalidating composed working view, per-repo status, rebase and teardown — the host keeps only sparse materialization tracking and the disk projector). The vcs read/history traversals are served by the DO's `vcs*` methods behind the `vcs` manifest service (userland dispatch via `workers.resolveService`); the host resolves the store DO itself from the SAME service declaration (`resolveVcsStoreBinding`; the `providers.vcsStore` manifest slot is deleted). The host `refs` RPC service (readMain/listMains/readMainLog + the single write `updateMains`, approval-gated — see the narrow-host VCS split, `docs/narrow-host-vcs-plan.md`) is the surface a userland VCS uses to read protected `main` refs and request advancement (the DO's refs bridge uses readMain for compose bases and listMains for workspace-view composition). `updateMains` is an atomic group compare-and-swap over `main` refs restricted to the single VCS writer (the gad-store DO); public `refs.advanceRef` and the generic `(repo, ref)` namespace are DELETED. Push/merge orchestration, FF/clean-source/divergence policy, and provenance recording moved INTO the DO (userland VCS): the DO composes + build-validates a candidate, CASes via `updateMains`, and records provenance in its own store via write-ahead publish intents (crash self-heal compares `listMains()` against those intents on restart). A `main`-TARGET merge flows through that same DO push path → `updateMains`, not a separate host advance. `ProvenanceFollower` (`src/server/vcsHost/provenanceFollower.ts`) survives ONLY for the scan/freshness path: adopting the user's own on-disk edits into `main` still records the transition in the DO after the fact (per-repo serialized, retried), with a ref→DO reconciler healing crash gaps at attach and on demand.
