...
- **Two approval systems** (GAD investigation):
  - **(a) Agent tool-call approvals** — already journaled in userland GAD:
    ...
    `resolved_by_json` (`packages/semantic-control-plane/src/index.ts:1833-1846`, `projectApproval`
    `:4008-4056`).
...
| `packages/semantic-control-plane/src/index.ts` | `projectApproval` reads account into `resolved_by_json`; `approval.resolved.v1` carries `actor.metadata.userId`                                                                        |
