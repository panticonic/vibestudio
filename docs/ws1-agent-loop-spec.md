...
foundation contract `docs/stage0-unified-log-spec.md` (the semantic control-plane API this
...
`vibestudio.blob-ref.v1` `StoredValueRef`s inside entries (hydration happens in
...
| --- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `turn.opened`                                                               | `openTurn == null` (duplicate-guard upheld by the semantic control plane) | `openTurn = {turnId: causality.turnId, openedAtSeq: seq, reason: payload.reason, modelCallCount: 0}`; `pendingPrompt = null` (its content is now in entries)                                                                                        |
...
executors, the alarm, and the semantic control-plane HTTP client. One driver per DO; one
...
The semantic control plane is remote (postToDO HTTP); without a cache every wake is an
...
command, `getAgentSettings` plus activation-local `getDebugState` (never
hydrating a fold for diagnosis),
...
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Streaming assistant text/thinking                  | durable per-delta trajectory publication                                                                                  | model executor → `onEphemeral` → batched (~100ms) channel `signal` broadcast of the identical AgenticEvent shape; `message.completed` carries authoritative blocks (§2.4.1)                                                                                                                                                                                                                                         |
...
4. **Driver.** `agentic-do/src/{agent-loop-driver,effect-outbox,fold-cache}.ts`
   - reduced `turn-dispatcher.ts` + crash-injection (§4.3) and cache-amnesia
     (§4.4) suites. Gate: both suites green against the simulated semantic control plane
