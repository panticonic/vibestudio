# Vibez1 — Electron Security Audit
...
`ServiceDispatcher.dispatch()` validates Zod args but never calls `checkServiceAccess`. The only places `checkServiceAccess` is invoked are `src/server/rpcServer.ts:496` and `:905` — the server's WebSocket RPC surface. The Electron-side dispatcher and the `vibez1:serviceCall` IPC handler ignore policy entirely:
...
1. In `ServiceDispatcher.dispatch()` (or in a pre-dispatch wrapper invoked from both `IpcDispatcher.handleMessage` and the `vibez1:serviceCall` handler) call `checkServiceAccess(service, ctx.callerKind, this, method)` before handler execution.
2. Double-check that `callerKind: "panel"` cannot be forged by a caller claiming `callerId === "shell"`. Currently `resolveCallerId` compares `event.sender.id` to `viewManager.getShellWebContents().id` — this is sound so long as no other code path sends `vibez1:serviceCall` with a spoofed `callerKind`. Good.
...
```ts
ipcMain.handle("vibez1:navigate", async (event, browserId: string, url: string) => {
...
ipcMain.handle("vibez1:goBack",    async (event, browserId: string) => { resolveCallerId(event); viewManager!.getWebContents(browserId)?.goBack(); });
ipcMain.handle("vibez1:goForward", ...);
ipcMain.handle("vibez1:reload",    ...);
ipcMain.handle("vibez1:stop",      ...);
```
...
Contrast with `vibez1:getCdpEndpoint` which correctly enforces ownership via `cdpServer.canAccessBrowser`. The parallel `view.browserNavigate` service method in `src/main/services/viewService.ts:53-57` has the same flaw.
...
### [HIGH-1] `vibez1:rpc:send` hardcodes `callerKind: "shell"` regardless of sender

**File:** `src/main/ipcDispatcher.ts:86-88`

```ts
ipcMain.on("vibez1:rpc:send", (event, targetId: string, message: unknown) => {
...
Practical exposure today is lower because panel preloads expose only `invoke` — not `send("vibez1:rpc:send", ...)`. However browser panels have `browserPreload.ts` with `__vibez1_autofill.ping()` which uses `send`, showing that panels can use `ipcRenderer.send` if it's ever exposed; and the autofill preloads import `ipcRenderer` directly. A single careless `contextBridge.exposeInMainWorld` refactor would expose `send` to an untrusted context and promote this to CRITICAL.

**Remediation:**

- Validate `event.sender.id === getShellWebContents()?.id` inside the handler. Reject otherwise.
- Do not accept the wildcard "vibez1:rpc:send" from any non-shell origin. Non-shell RPC should use `vibez1:serviceCall` exclusively.
...
// autofillOverlay.ts
ipcMain.on("vibez1:autofill-overlay:select", (_event, id: number) => {
  this.callbacks?.onSelect(id);
});
ipcMain.on("vibez1:autofill-overlay:dismiss", () => {
  this.callbacks?.onDismiss();
});
```

`vibez1:autofill:ping` uses `event.sender.id` so it only acts on the caller's own state — the risk is limited to a panel spoofing "my autofill content script is running" to induce credential loading. Still, it causes `passwordStore.getForOrigin(currentOrigin)` to run and can cause fill attempts; a panel can trigger a fill into itself even if the content script was never injected.

`vibez1:autofill-overlay:select` and `vibez1:autofill-overlay:dismiss` are the worse case. `onSelect(credentialId)` calls `handleOverlaySelect` which scans all panels and fills the credential into whichever panel has it loaded. There's no check that the event came from the overlay view, that the overlay is currently visible, or that `credentialId` belongs to the visible panel. An attacker-controlled renderer that can reach `ipcRenderer.send` can iterate credential IDs and force fills into any panel that currently has credentials loaded — and then read them back through the DOM.

Today, the panel preload (`panelPreload.ts`) does not expose `ipcRenderer.send`. But the browser preload (`browserPreload.ts:10`) exposes `__vibez1_autofill.ping()` which wraps `ipcRenderer.send("vibez1:autofill:ping")`. If an attacker controls content in a browser panel (which they do — browser panels load arbitrary external sites) and can chain to `ipcRenderer.send` with a different channel name, this is direct credential theft. Review indicates the overlay preload (`autofillOverlayPreload.ts`) is only injected into the overlay WebContentsView, so content-page JS can't reach it. But `ipcRenderer.send` with any channel name is a single typo away from being reachable if the preload shape ever changes.
...
### [HIGH-3] `vibez1:openExternal` allows arbitrary http/https including javascript-scheme exfil via user's default browser

**File:** `src/main/index.ts:803-816`

```ts
ipcMain.handle("vibez1:bridge.openExternal", async (_event, url: string) => {
  if (!/^https?:\/\//i.test(url)) {
...
The scheme check is good. But any panel can trigger `shell.openExternal(anyUrl)` with no user confirmation, causing the OS default browser to open an attacker URL. While the attacker doesn't gain execution in Vibez1, they can:
...
- Serve phishing that looks like it came from Vibez1.
...
### [HIGH-4] `vibez1:openFolderDialog` / `openFileDialog` have no caller restriction

**File:** `src/main/index.ts:778-801`

```ts
ipcMain.handle("vibez1:bridge.openFolderDialog", async (_event, opts?: { title?: string }) => {
...
```ts
if (process.env["VIBEZ1_TEST_MODE"] !== "1") { return; }
global.__testApi = { ... };
```
...
### [LOW-3] Shell preload sets `__vibez1Transport` on `globalThis` (shell is contextIsolation: false)

**File:** `src/preload/index.ts:22`

```ts
globalThis.__vibez1Transport = shellTransport;
```
...
3. **Replace the catch-all `vibez1:serviceCall` design with a service-method allowlist keyed by caller kind.** Generic dispatch + blanket policy is brittle.
4. **Remove the global CORS-strip on the default session.** Route panel outbound HTTP through the egress proxy.
5. **Flip the shell to `contextIsolation: true, sandbox: true, nodeIntegration: false`** and expose the minimum required API via a contextBridge in `src/preload/index.ts`. Migrate `__vibez1Transport` to an `exposeInMainWorld` call.
