...
Vibestudio runs agent-authored TypeScript on three rails:
...
- CDP authentication no longer uses URL query strings. `getCdpEndpoint` returns `{ wsEndpoint, token }`; browser WebSocket clients open the token-free URL and send a first-frame `{ type: "vibestudio:cdp-auth", token }` message. This is intentional because browser `WebSocket` cannot set `Authorization: Bearer` headers.
...
**Remediation**: set `Access-Control-Allow-Origin` to the known Vibestudio origin(s) (the panelHttpServer origin), not `*`. Drop the CORS headers entirely for unsafe methods.
...
1. User opens `https://bank.com` in a Vibestudio browser panel. Autofill stores `user@bank` / `hunter2`.
...
4. Vibestudio's AutofillManager (S3) injects bank creds into the iframe's main-world DOM.
