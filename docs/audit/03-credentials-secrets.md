...
Branch: `audit` · Target: Vibestudio credential subsystem (wave 6–8 recent build)
...
Vibestudio now mediates OAuth / API credentials for Gmail, Calendar, GitHub,
...
| # | Severity | Issue |
|---|----------|-------|
| F-01 | **Critical** | `CredentialStore` writes OAuth access + refresh tokens as **plaintext JSON** under `~/.config/vibestudio/credentials/` — no OS keychain, no encryption-at-rest. |
...
| F-06 | **High**    | Third-party provider manifests (`@someone/vibestudio-provider-foo`) are trusted unconditionally — no signing, no allow-list — and control `apiBase` (→ which hosts get `Authorization: Bearer <user-token>` stamped) and `tokenUrl` (→ where the refresh-token is POSTed). |
...
| F-08 | **Medium**  | Full request URLs (with query strings) persisted to audit log (`~/.config/vibestudio/logs/credentials-audit-YYYY-MM-DD.jsonl`). Some provider APIs put auth codes / tokens / sensitive IDs in query strings. Log file created with default mode (not `0o600`). |
| F-09 | **Medium**  | Panel bootstrap gateway caller token is persisted in `sessionStorage` (`__vibestudioPanelInit`) and exposed as `globalThis.__vibestudioGatewayToken` / `__vibestudioGatewayConfig` — any script in the panel's origin (incl. supply-chain-poisoned npm deps bundled into the panel) can read it. |
...
1. Any other process running as the user can read
   `~/.config/vibestudio/credentials/**/*.json` (on Linux/Mac) — no privileges needed.
...
3. Directory is under the standard central Vibestudio data directory
   (`$XDG_CONFIG_HOME/vibestudio` or `~/.config/vibestudio` on Linux).
4. Core dumps / process memory snapshots include the plaintext tokens after
   load.
5. A `.config/vibestudio/` glob shared with a coworker (e.g. via a shared home
...
- In server-standalone mode (no Electron available), use a KEK derived from
  ...
  lives in `~/.config/vibestudio/.creds-kek` (0o600) so at least it is in a
...
- Move the default base path under the OS-appropriate state directory
  (macOS: `~/Library/Application Support/vibestudio/credentials`, Linux:
  `$XDG_STATE_HOME/vibestudio/credentials`) alongside other state — the existing
...
```js
await vibestudioRpc.call("authTokens.getProviderToken", ["anthropic"]);
// → "sk-ant-api03-…"
```
...
every Vibestudio user who installs the panel.
...
const WORKER_ID_HEADER = "x-vibestudio-worker-id";
const PROXY_AUTH_HEADER = "x-vibestudio-proxy-auth";
...
2. Send:
   ...
   X-vibestudio-Worker-ID: my-chosen-worker-id
   X-vibestudio-Proxy-Auth: my-chosen-caller-id
   ```
...
secret header. Vibestudio mints that secret and then forgets to check it.
...
containers with multiple UIDs), `cat ~targetuser/.config/vibestudio/.secrets.yml`
...
> `@someone/vibestudio-provider-foo` with their own `client_id` and a
...
- Require operators to explicitly allow-list third-party provider IDs in
  central config (`~/.config/vibestudio/config.yml` → `providers: [ ... ]`),
  default deny.
...
1. Other users on the machine can `tail -f ~/.config/vibestudio/logs/credentials-audit-*.jsonl`
...
...
globalThis.__vibestudioGatewayToken = cfg.gatewayConfig.token;
globalThis.__vibestudioGatewayConfig = cfg.gatewayConfig;
```

The panel gateway caller token is:
- Injected into the page global namespace. Any script on the page origin
  reads it via `globalThis.__vibestudioGatewayToken`.
...
against Vibestudio users.

**Remediation.**

- Keep the token in closure-scoped state inside a preload/isolated-world
  script; wrap RPC calls through a narrow `window.vibestudio.rpc.call(...)`
...
- Or, encrypt with a machine-KEK derived from a file readable only by the
  user (`~/.config/vibestudio/.kek`) as a better-than-plaintext fallback.
...
- The nonce travels through the redirect URL, into OS URL handlers / browser
  ...
  intercepts `vibestudio://oauth/callback`) can pick it up.
...
links and custom URL schemes (F-05's `vibestudio://`), hijacking the URL scheme
...
- `packages/browser-data/` — uses the documented Chromium / Firefox legacy
  ...
  public spec for reading existing browsers' cookie stores, not Vibestudio's
