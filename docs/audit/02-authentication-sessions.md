# Vibestudio Security Audit — Authentication, Sessions & Authorization

**Audit date:** 2026-04-23
**Branch:** `audit`
**Scope:** Authentication, session management, and authorization across the Vibestudio
...
Vibestudio's auth model is built around a two-tier bearer-token system: a single
**admin token** (32-byte hex, persisted at `~/.config/vibestudio/admin-token` with
...
the resulting credentials at `~/.config/vibestudio/oauth-tokens.json` (`0o600`).
...
2. **[Critical] Path traversal in `CredentialStore`.** `providerId` and
   ...
   panel with access to `credentials.*` methods can escape `~/.config/vibestudio/credentials/`.
...
4. **[High] Egress proxy trusts caller-supplied attribution headers.** The
   ...
   (`x-vibestudio-worker-id`, `x-vibestudio-proxy-auth`) that it never validates
...
6. **[High] Mobile OAuth callback deep-link registry is never consumed.**
   ...
   call sites — `LoginScreen.tsx` only consumes `vibestudio://connect` links,
   leaving `vibestudio://auth/callback` deep-links ignored. Beyond the
...
- `persist(providerId, credentials)` — writes attacker-controlled OAuth
  credentials to `~/.config/vibestudio/oauth-tokens.json` (`0o600`) under any
...
`~/.config/vibestudio/admin-token`, `oauth-tokens.json`, the user's shell
...
outgoing HTTP via `fetch`) sets `X-Vibestudio-Worker-Id: <target-worker-id>`
and `X-Vibestudio-Proxy-Auth: anything` and gets outbound calls signed
...
`vibestudio://connect` deep-links. The Codex OAuth redirect URI is
`vibestudio://auth/callback` (`codexAuthFlow.ts:17`), so when the OS
...
2. *Timing/DoS.* An attacker that can fire `vibestudio://` intents on
   ...
   …)` branch is fired on any `vibestudio://connect` URL — cooperative
...
- Because any installed app can fire `vibestudio://auth/callback`,
...
- Add universal links (`https://`) so the redirect is OS-verified to
  the Vibestudio app rather than racy custom-scheme delivery.
...
switch (params.redirect) {
  ...
  case "mobile-universal":
    redirectUri = "vibestudio://oauth/callback";
    break;
}
```
...
- `mobile-universal` uses a custom URL scheme
  (`vibestudio://oauth/callback`) — OK only if universal links (verified
...
- A *ToS surface.* OpenAI may change their allow-list at any time, and
  ...
  revokes the client, all Vibestudio Codex users lose access. More
  importantly, if OpenAI considers this impersonation a ToS violation,
  users could be penalized.
- A *supply-chain data-point.* Vibestudio traffic to OpenAI is
...
Vibestudio and use its id + allowlisted `originator`.
...
- **Admin token header form** (`X-Vibestudio-Token`) is preferred over
...
- **Timing-safe HMAC comparison** for webhook signatures
  ...
  are missing (§2.9).
- **Deep-link validation** for `vibestudio://connect` on mobile
...
13. **Low priority:** register a first-class OpenAI OAuth client for
    Vibestudio rather than impersonating Codex CLI (§2.13).
