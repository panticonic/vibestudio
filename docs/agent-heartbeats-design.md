...
- **Cron jobs**: deterministic calls into a Durable Object, declared in
  `meta/vibestudio.yml` `recurring:` and approval-gated before they can run.
...
OpenClaw's public bug history also highlights two failure modes vibestudio should
avoid:
...
- `meta/vibestudio.yml` `heartbeats:` parsing, validation, and approval entries;
...
unattended scheduled agent work in `meta/vibestudio.yml`. That is a safety
...
### 5. `meta/vibestudio.yml` Declarative Heartbeats
...
string like `bgkit-${channelId}` in `meta/vibestudio.yml`; they can register their
