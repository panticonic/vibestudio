...
| WP3 remote deploy            | Local or SSH deploy/pairing/status/log/update/remove automation uses one systemd user-service lifecycle and version-pinned artifacts. Managed pairing reads protected ready state and deployment proves routed workspace health.                            | `scripts/cli/remote-deploy.mjs`, `src/cli/client.ts`, `docs/cli.md`, `workspace/skills/remote-access/SKILL.md`                                                                                                                                    |
...
vibestudio remote deploy pairing|status|logs|update|remove local
