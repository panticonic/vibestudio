...
| WP6 HTTPS pair carrier | Pair URLs use the HTTPS carrier, app-link metadata is served by the apex relay Worker, and fallback behavior requires explicit user action. | `scripts/cli/lib/connect-utils.mjs`, `apps/webhook-relay/src/index.ts`, `apps/webhook-relay/src/oauthLanding.ts`, `apps/mobile/android/app/src/main/AndroidManifest.xml`, `apps/mobile/ios/Vibestudio/Info.plist` |
