...
| No-copy log fork at a seq                       | ✅                     | semantic control plane `forkLog` (`workspace/workers/workspace-source/GadWorkspaceDO.ts`), `ChannelLog.forkFrom` (`pubsub-channel/log-store.ts:147`)                                                                                                                      |
...
- **`mode: "fresh"`** — a new agent (configurable worker source, default the standard chat agent; model/thinking/approval config in `stateArgs.agentConfig`). It starts with an isolated LLM trajectory. Cheap and focused.
...
Both modes require one non-empty `task`. It is part of the child's durable
identity and appears in every immediate model prompt, so inherited history,
context trimming, retries, and later progress cannot make the child forget why
it exists.
...
merge_subagent({ runId, resolutions? })         // §B5; net-effect coordinate merge
close_subagent({ runId, discard?: boolean })    // retryable teardown; release slot and retain a closed receipt on success
...
Depth/fan-out is guarded by config (`maxSubagentDepth`, `maxSubagents`), enforced at spawn time. `maxSubagents` counts child contexts still owned by the supervisor; a terminal run retains its slot until `close_subagent` successfully releases it, preventing accidental replacement groups while integration is pending. Close is a retryable lifecycle transition: launcher release and context destruction are both attempted, and the receipt becomes `closed` (releasing its slot) only when required teardown succeeds. A structured cleanup failure leaves the terminal receipt and slot owned so the same close call can safely retry. Closing preserves a bounded, handle-resolvable lifecycle receipt in the supervisor's per-model-call ledger, so releasing resources never erases the fact that delegation already happened. The existing `agentHopLimit` machinery (`addressing.ts:118`, `DEFAULT_AGENT_HOP_LIMIT = 4` at `:45`) prevents chat loops on the task channel.
...
- **`say` local tool** — explicit mid-turn publication: emits a durable channel message flagged `saliency: "say"`, which passes the supervisor's `"explicit"` wake filter. This is the subagent's "progress worth reporting" valve.
...
**Adoption uses the shared net-effect merge.** The take-everything action asks
`vcs.merge` to adopt bounded stable-coordinate pages from the same committed
event. Review may instead resolve named coordinates.
Each call:
...
A parent may keep compatible merged work while declining or deferring
...
- `merge_subagent({ runId, resolutions })` maps take-everything and explicit
  coordinate resolutions to one or more `vcs.merge` calls.
...
- The result is reported as `merged`, `needs-decision`, or `discarded`.
  A needs-decision result means non-conflicting selections may already be present
...
mutate each side, commit the child locally, compare the child event, merge
...
- **"Review changes"** on the SubagentRunCard opens the shared semantic diff-review UI fed by `vcs.compare` (§B5). File and event rows retain their stable change identities, so the user can merge everything incrementally, choose named changes, reconcile or decline a change, and inspect typed conflicts without reducing intent to paths or patches. The same review action appears on every authorized entry in the fork switcher/tree.
...
- Terminal states: success (report delivered) → parent decides merge / discard; failed / cancelled similarly (§B1 terminal-event mapping). `close_subagent` unsubscribes both sides, releases an extension-owned launcher when present, and destroys the child context. It releases the owned slot and records a local closed receipt with the same public handle only after required teardown succeeds; a structured cleanup failure preserves the terminal receipt for an idempotent retry with the same handle. Live workspace/transcript inspection ends at successful close, while lifecycle/config/integration metadata remains queryable and is projected into every supervisor model call.
...
| `agent-loop`                | `publishPolicy` gating in effects derivation (**generalizes + deletes existing `silentPolicy()`**); `say` tool (**generalizes + deletes existing `createSayTool`**); `wakePolicy` field (resolution lands in the vessel, not here); `maxSubagentDepth`/`maxSubagents` in config                                                                                                                                                                    | B3, B4         |
| `agentic-do` (vessel)       | **C3: per-channel fork** — `canFork`/`postClone` reworked to fork one subscription and drop the rest (delete the ≤1-subscription gate); `initFromTrajectoryFork` init path; `spawn_subagent`/`send_to_subagent`/`inspect_subagent`/`merge_subagent`/`close_subagent` tools; `wakePolicy` resolution + explicit-only supervisor wake for `wakePolicy: "explicit"`                                                                               | A6, B2, B3, B5 |
...
| Semantic control plane      | O(1) context fork by sharing the committed event and exact working head; authorization reachability through context relationships; bound `compare`/`merge`/decision/inspector traversal over the one semantic workspace graph                                                                                                                                                                                                                      | B5, A1         |
...
- Cost is proportional to owned subagent contexts, which is bounded by `maxSubagents`; fork remains no-copy at the log layer either way.
...
vcs.compare({ target: parent.workingHead, source: { kind: "event", eventId: sourceEventId } })
vcs.merge({ commandId, contextId: parentContextId,
            expectedWorkingHead: parent.workingHead,
            source: { kind: "event", eventId: sourceEventId } })
// Repeat bounded coordinate pages against each returned working head, then commit the
// complete parent application chain. Push remains a separate user choice.
inspect_subagent({ runId, query })
merge_subagent({ runId, resolutions })
// agent-loop config additions (AgentLoopConfig at state.ts:39 has none of these today)
AgentLoopConfig += { publishPolicy: "all" | "turn-final" | "say-only"; maxSubagentDepth: number; maxSubagents: number }
...
SubscriptionConfig += { wakePolicy: "every-envelope" | "explicit" | "manual" }
...
merge_subagent({ runId, resolutions })  // vessel (WS-5): accounts for source coordinates (§B5)
...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **WS-1 semantic VCS use**       | O(1) context pointer fork, authorized cross-context state reachability, semantic compare/merge/inspect flow, and complete-chain commit behavior. Implement against the canonical provenance-aware VCS contract; do not recreate merge, pick, head, or patch abstractions locally.                                                                                                                                                                                                                                                                                                                                                                                                | `workspace/workers/workspace-source/`, `workspace/packages/agentic-do/`                           |
...
| **WS-5 vessel**                 | Per-channel fork (`canFork`/`postClone` rework, ≤1-subscription gate deleted); `initFromTrajectoryFork`; `spawn_subagent`/`send_to_subagent`/`inspect_subagent`/`merge_subagent`/`close_subagent`; explicit-only supervisor wake plus durable progress projection; explicit lifecycle/GC                                                                                                                                                                                                                                                                                                                                                                                     | `workspace/packages/agentic-do/`                                                                  |
...
2. Verification suite — all must pass:
   ...
   - Parent spawns fresh + forked subagents; a fork's first task prompt explicitly marks inherited parent/user instructions as reference context and the enclosed delegated task as its sole active assignment; supervisor wakes only on explicit `say`, direct address, or terminal completion; steers mid-run via `send_to_subagent`; human inspects + posts via child panel; `close_subagent` tears down entity + context while retaining the durable lifecycle receipt.
...
7. **Only semantic VCS verbs remain**: subagent callers use `status`/`compare`/`merge`/`inspect`/`commit`; no patch-transport or repository-local ancestry API family exists. Tool results and UI labels use `merged | needs-decision | discarded`. The canonical provenance-aware plan is the sole VCS contract.
...
- **Supervisor observation** (`wakePolicy: "explicit"`) keeps the task-channel log authoritative across DO hibernation. Ordinary progress projects durably onto the parent invocation card; only explicit messages wake the parent model.
...
- **No local VCS dialect** (WS-1/WS-2/WS-5): import canonical semantic schemas and delegate to `compare`/`merge`/inspectors. Do not recreate merge gates, patch application, source/target heads, path picks, conflict-marker lifecycle, or convenience authority in the vessel or host.
