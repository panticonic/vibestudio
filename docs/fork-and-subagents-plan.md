...
Authoritative background specs: `docs/ws1-agent-loop-spec.md` (agent loop), `docs/ws2-channel-spec.md` (channel DO), `docs/stage0-unified-log-spec.md` (gad log + forkLog), `docs/narrow-host-vcs-plan.md` (host/gad VCS split — P3+ pending). `docs/agentic-architecture.md` and `docs/pi-architecture.md` are historical; do not trust their specifics.
...
| No-copy log fork at a seq | ✅ | gad-store `forkLog` (`workspace/workers/gad-store/index.ts:473`), `ChannelLog.forkFrom` (`pubsub-channel/log-store.ts:147`) |
...
| **File-context fork incl. uncommitted changes** | ✅ **confirmed** | `runtime.cloneContext` → `WorkspaceVcs.forkContext` (`src/server/vcsHost/workspaceVcs.ts:1441`): snapshots the composed working view (committed ctx head ⊕ uncommitted edit-ops) and pins it as the child's base |
...
2. **Fork lineage is one-directional and buried.** The child records `forkedFrom`/`forkPointId` in its KV `state`, reachable only via generic `getState()`. The **parent has no record that it was forked** — no parent→children index, no way to enumerate a fork tree, no event anyone can subscribe to. This is the core structural gap: forks are invisible.
3. **No notification.** Other participants (human or agent) never learn a fork happened.
4. **Edit is unread-only.** `PubSubClient.editMessage` + `message.edited` apply only to messages not yet read (outbox editing, `useChatCore.ts:907`). Editing *read* history is semantically a fork, not an edit — the primitives are unrelated and should stay unrelated. Edit-a-past-message must be built on fork-at-seq, not on `message.edited`.
...
6. **VCS lineage is flattened on fork — a defect to fix, not a caveat to accept.** `forkContext` (workspaceVcs.ts:1441) snapshots the parent's *working view* and pins it as the child's clean base: the child loses commit lineage (`getMergeBase`, gad-store `:5842`, walks the recorded transition graph and finds no shared ancestor), and — worse semantically — the parent's *uncommitted* edits get **baked into the child's base as if committed**, so the child cannot see, discard, or selectively commit them. Harmless for the news deep-dive; wrong for edit-forks and fatal for subagent merge-back. §B5 replaces this with a true lineage fork.
...
- `reduceChannelView` (`reducer-channel.ts`) folds these into `ChannelViewState.forks: ForkProjection[]` — the fork roster is a pure projection of the parent log, exactly like the trajectory `branch.*` projections it mirrors.
- The **child** side already records `forkedFrom`/`forkPointId`; promote this to a typed **`getProvenance()`** RPC on the channel DO — a discriminated union covering *every* way a channel comes to exist, not just forks: `{ kind: "root" }` | `{ kind: "fork", forkedFrom, forkPointId, rootChannelId }` | `{ kind: "task", parentChannelId, parentContextId, runId }` (task channels are *fresh* channels, §B1 — their provenance is recorded at creation, not inherited from a log fork). Panels landing anywhere can render breadcrumbs and walk *up*.
...
- gad-store `forkLog` already persists `parent_log_id`/`fork_seq`; expose it as `getLogLineage(logId)` for tooling/debugging so the durable substrate and the channel-level events can be cross-checked.
...
3. **reconciles on alarm/wake**: an op found mid-phase resumes from its journal (every phase is idempotent — the clone is keyed by forkId, the appends by deterministic envelopeIds `fork-seed:{forkId}` / `fork-event:{forkId}`) or, if unresumable, rolls back via `destroyContext`. No crash window can leave either an advertised-but-empty fork or an unadvertised live clone.
...
The **seed** carries the replacement message: author (the forking user), content blocks, and the optional `replaces: { messageId, seq }` marker (§A2). It lands via a new **service-authored append** on the forked channel — `appendSeed`, caller-asserted to the owning fork op (the same DO-trust plane as `postClone`) — writing a normal *primary* user-authored message envelope with an explicit `onBehalfOf` actor and audit fields, **without requiring a registered participant** (the clone's participants table is empty post-fork by design; the participant assertion on the ordinary `publish` path is untouched). This is NOT the `forceInitialPrompt` mechanism — that sends `tier: "secondary"` system-originated support prompts and cannot carry `replaces`; it stays reserved for its existing new-chat use.
...
1. **Per-channel fork for multi-subscription vessels** (prerequisite for Part B coexistence, §C3): replace the `subscriptions.count() <= 1` gate. `postClone(parentChannelId, …)` becomes "fork *this* subscription": re-root the trajectory for the forked channel, rename that subscription, and **drop all other subscriptions in the clone** (the clone is a new entity; it must not ghost-join the parent's other channels). `canFork` then only vets per-channel invariants (e.g. no open method calls it cannot reconcile — existing `reconcilePendingCalls` logic).
...
1. **Fork the parent's file context** → child context (uncommitted changes intact, via `forkContext`), with fork provenance recorded (§B5).
2. **Create a task channel** `task-{uuid}` bound to the child context.
3. **Create the child agent entity** in the child context, `parent` link set (`runtime.createEntity({ parent })` — the ownership edge already exists, `runtimeService.ts:56`), and subscribe it to the task channel.
4. **Subscribe the parent** to the task channel with a supervisor wake policy (§B3).
5. Record the whole run on the **parent trajectory as a durable invocation**, using the existing terminal vocabulary exactly (no semantic overload): the spawn tool emits `invocation.started`; child `say` messages and its end-of-turn report surface as `invocation.output`; success closes with `invocation.completed` (which is success-only by schema — `terminalOutcome: "success"`), a failed run closes with `invocation.failed`, a parent-aborted run with `invocation.cancelled`, and a TTL-swept/orphaned run with `invocation.abandoned`. Merge status rides in the terminal payload's subagent block. The parent's LLM context therefore contains the subagent run in the shape it already understands — an async tool call with streamed progress and standard error folding (`isError` behaves correctly for free).
...
Depth/fan-out guarded by config (`maxSubagentDepth`, `maxConcurrentSubagents` in `AgentLoopConfig`), enforced at spawn time; the existing `agentHopLimit` machinery already prevents chat loops on the task channel.
...
Add a per-subscription **`wakePolicy`** to the subscription config, resolved in `@workspace/agent-loop` policies alongside `RespondPolicy`:
...
Today every model call's text becomes a `message.completed` envelope on the channel. For subagents, add to `AgentLoopConfig`:

- **`publishPolicy: "all" | "turn-final"`** — under `"turn-final"`, intermediate model text within a turn is retained in the trajectory (`message.completed` trajectory events, streamed as ephemeral deltas so live inspectors still see progress) but **only the end-of-turn message is published as a durable channel envelope**.
...
Subagent default config: `publishPolicy: "turn-final"` + `say` in the tool roster. This is a pure agent-loop/fold change (new effect gating in `derivePendingEffects` + one tool), and it is generally useful beyond subagents (e.g. quieter agents in crowded human channels).

### B5. VCS: lineage-true context fork + generalized merge

**Fork — rework `forkContext` into a first-class lineage operation.** Today's snapshot-and-pin flattens history and bakes the parent's uncommitted edits into the child's base (§0.6). Instead of patching around that (e.g. bolting synthetic transitions onto `getMergeBase`), fix the primitive. New `forkContext` semantics, per repo touched by the parent:
...
**Merge — generalize the surface (§G2).** The DO engine already merges arbitrary `sourceHead` into a `ctx:*` target (`vcsMerge`, `index.ts:6569`; `main` as target rejected — correct, that path stays reserved for gated pushes). The limitation is caller-facing: `vcs.merge` hard-pins source = `main` (`vcsService.ts:466`). Replace it (no compatibility kept):
...
- `source: "main"` preserves today's pull-main behavior; `source: { contextId }` merges another context's committed ctx-head states in, per touched repo: `vcsMerge({ logId, targetHead: ctx:<caller>, sourceHead: ctx:<source>, actor })`. Merge base = `getMergeBase` over the lineage graph, which the lineage-true fork keeps connected.
...
- **Conflicts** reuse the existing pending-merge machinery unchanged (provisional conflict-marked tree, resolve via `vcs.edit` + `vcs.commit`, `vcsAbortMerge`); the result surfaces on the parent's invocation as `conflicted`.

**Merge-back is commit-gated on both sides (§G2, final).** `vcsMerge` requires committed states on source and target, and that stays the contract:
...
**Cross-context inspection.** The read surfaces (`readFile`, `listFiles`, `diff`, `log`, `status`, `contextStatus`, `pendingMerge`) gain a `context: { contextId }` scope: resolve against *that* context's heads/working states instead of the caller's. Authorization walks the **context-relationship registry** (new, §B7 — typed edges, and it accepts **either** kind: `lifecycle` for subagents, `lineage` for forks), enforced at the existing `context.boundary` gate: a caller may inspect contexts it owns or forked — and the shell/user may inspect anything of theirs. New convenience projection `vcs.contextDiff({ contextId, against?: "fork-base" | "main" })` — the full working-state diff of another context against its fork point (default) or main; this is the "review the subagent's work" surface and feeds the existing diff-review UI (narrow-host P3.5) unchanged.
...
- **A real context-relationship registry is built as part of this plan** — today's `parentId` is launch-parent metadata on *entities* only; `destroyContext` scans a single context's entities and traverses nothing (`runtimeService.ts:570`). New: the runtime persists a typed edge on every context created by `cloneContext`/subagent spawn (alongside the context, not derived from entity records), with **two edge kinds that must not be conflated**:
  ...
  `listOwnedContexts(contextId, kind?)` exposes the edges; `destroyContext({ recursive: true })` does post-order teardown of the **lifecycle** subtree only (and is the default for contexts with lifecycle children); §B5b inspection authorization accepts **either** edge kind. One registry, three consumers, edge-kind-aware.
- Terminal states: success (report delivered) → parent decides merge / discard; failed / cancelled similarly (§B1 terminal-event mapping). `close_subagent` unsubscribes both sides, destroys the entity, and destroys the context — **unless** the run is kept for inspection (default: keep until merged-or-discarded, then destroy on `close_subagent`; a supervisor-config TTL sweeps abandoned children, closing their invocations as `invocation.abandoned`).
- Parent death: destroying the parent's context with `recursive: true` (the default for contexts with lifecycle children) tears down the whole subagent tree via the registry's lifecycle edges — conversation forks (lineage edges) are untouched; children that survive an explicit non-recursive teardown are surfaced in the panel via their invocations' non-terminal state and swept by TTL.
...
| `agent-loop` | `publishPolicy` gating in effects derivation; `say` tool; `wakePolicy` in subscription policy resolution; `maxSubagentDepth`/`maxConcurrentSubagents` in config | B3, B4 |
| `agentic-do` (vessel) | **C3: per-channel fork** — `canFork`/`postClone` reworked to fork one subscription and drop the rest (delete the ≤1-subscription gate); `initFromTrajectoryFork` init path; `spawn_subagent`/`send_to_subagent`/`merge_subagent`/`close_subagent` tools; supervisor buffering for `wakePolicy: "turn-final"` | A6, B2, B3 |
| `pubsub-channel` | `getProvenance()` typed RPC (root/fork/task union); **durable `fork_ops` journal + `fork()` RPC + alarm reconciler** (the fork operation moves into the parent channel DO); `appendSeed` service-authored append; service-authored fork-event appends | A1, A3 |
...
| `gad-store` | **Lineage-true `forkContext`**: ctx-head fork edges (forkLog mechanics applied to context heads), edit-op copy to child head, `forkProvenance` metadata; `getLogLineage()`; `vcsPick` (forward dual of revert's patch application + provenance-tagged edit-op injection for path picks) | B5, B5b, A1 |
| Host `vcs` service | Generalized `vcs.merge({ source })` (main **or** another context; old main-pinned signature deleted); `context: { contextId }` scope on read surfaces + `vcs.contextDiff`; `vcs.pick` (commit + path cherry-pick) | B5, B5b |
| `runtime` / host | `cloneContext({ recursive })` — compositional clone over the context ownership tree with parent/subscription/provenance rewiring | B1, B6, D |
...
- In-flight method calls / open invocations across the boundary are reconciled by the same `reconcilePendingCalls` machinery `postClone` already runs — each cloned pair re-establishes its own pending state; calls that cannot be re-homed settle as `aborted-by-fork`.
...
subscribeLineage(participantId)   // signal-only subscription at the tree root
...
cloneContext({ sourceContextId, include?, recursive?: boolean }): {
  contextId: string
...
PubSubChannel.fork({ forkPointPubsubId, seed?: { author: ParticipantRef; blocks: Block[];
                     replaces?: { messageId: string; seq: number } }, label?, reason, exclude?, replace? })
// fork_ops journal row first; phases idempotent (clone keyed by forkId; appends by deterministic
// envelopeIds fork-seed:{forkId} / fork-event:{forkId}); alarm-driven reconciler resumes or rolls back.
...
appendSeed(forkOpRef, envelope)   // service-authored PRIMARY append on the forked channel: caller-asserted
                                  // to the owning fork op (postClone trust plane), explicit onBehalfOf actor
                                  // + audit fields, no participant registration required
...
// context?: { contextId } — authorized via the runtime ownership tree (context.boundary gate)

// agent-loop config additions
AgentLoopConfig += { publishPolicy: "all" | "turn-final"; maxSubagentDepth: number; maxConcurrentSubagents: number }
...
// parent abort → invocation.cancelled; TTL sweep/orphan → invocation.abandoned.
...
// terminal payloads (completed/failed/cancelled/abandoned) all carry:
...
|---|---|---|
| **WS-1 gad substrate** | Lineage-true `forkContext` (ctx-head fork edges via forkLog mechanics + edit-op copy + `forkProvenance`); `getLogLineage`; `vcsPick` engine (commit-patch forward-apply + path-level working-content injection) | `workspace/workers/gad-store/` |
| **WS-2 host VCS** | Generalized `vcs.merge({ source })`, old main-pinned signature deleted, **all callers updated** (CLI `vcsCommands`, panels, skills); precise dirty-side error surfaces for commit-gated merge-back; `context: { contextId }` scope on read surfaces + `vcs.contextDiff` + `vcs.pick` dispatch with ownership-tree authorization | `packages/shared/serviceSchemas/vcs.ts`, `src/server/services/vcsService.ts`, `src/server/vcsHost/` |
| **WS-3 runtime** | Context-relationship registry (typed `lifecycle`/`lineage` edges, `listOwnedContexts`); `destroyContext({ recursive })` post-order teardown of the lifecycle subtree; `cloneContext({ recursive })` per the §E1 contract: lifecycle-subtree walk, root-only `include`, full descendant clone, rewiring map, cloned-child re-parenting, pending-call re-homing, error on non-recursive-with-lifecycle-children | `src/server/services/runtimeService.ts`, `packages/shared/src/runtime/` |
| **WS-4 agent-loop** | `publishPolicy` gating in effects derivation; `say` + `read_subagent` tools; `wakePolicy` resolution; depth/fan-out limits | `workspace/packages/agent-loop/` |
| **WS-5 vessel** | Per-channel fork (`canFork`/`postClone` rework, ≤1-subscription gate deleted); `initFromTrajectoryFork`; `spawn_subagent`/`send_to_subagent`/`inspect_subagent`/`merge_subagent`/`pick_from_subagent`/`close_subagent`; supervisor turn-final buffering (log-derived, replay-safe); lifecycle/GC + TTL sweep | `workspace/packages/agentic-do/` |
| **WS-6 channel + fork service** | `getProvenance` (root/fork/task); lineage-signal hub (`subscribeLineage`, head-advance reporting to root, `fork.head_changed` fan-out); durable `fork_ops` journal + `fork()` RPC + alarm reconciler in the parent channel DO; `appendSeed` service-authored append; **delete `workspace/workers/fork`**; `forkConversation` client helper; news-panel migration | `workspace/workers/pubsub-channel/`, `workspace/packages/channel-fork/`, `workspace/panels/news/` |
...
2. Verification suite — all must pass:
   ...
   - Quiet agent (`publishPolicy: "turn-final"`) publishes only end-of-turn + `say`; trajectory retains intermediate text; live inspector sees ephemeral deltas.
   ...
   - Fork with live subagents: recursive clone walks lifecycle edges only, clones descendant contexts **in full** despite a root-scoped `include`, rewires lifecycle edges, task-channel subscriptions, provenance, and pending calls; both trees operate independently afterward.
   ...
   - Fork switcher, tree view, and **live** badges: a message appended in a sibling fork badges the switcher via `fork.head_changed` without any panel poll; repeated cross-context panel navigation keeps stateArgs/storage isolation intact.
...
3. **The fork worker is deleted**; forking becomes a durable, journaled `fork()` RPC on the parent channel DO that *also appends* `channel.forked` to the parent log — forks become visible to all parent subscribers, and any consumer assuming forks are silent or worker-driven (none known beyond news) changes behavior.
...
7. **`vcs.merge` signature replaced**: `merge({ source: "main" | { contextId } })`; the implicit source-=-main form is deleted. Merge semantics (commit-gated, existing clean/conflicted behavior) are unchanged. All callers (CLI `vcsCommands`, panels, skills) updated in the same change.
8. **`forkContext` semantics change**: child is no longer a flattened clean pin — it inherits the parent's pinned base, a lineage-descendant ctx head, and the parent's uncommitted edits *as uncommitted edits*. Anything that assumed a freshly forked context reports clean status (news deep-dive, fork tests, `vcsContextStatus` consumers) sees different — truthful — status. `cloneContext` callers unaffected in content, affected in status.
...
10. **Trajectory event vocabulary**: subagent runs introduce invocation payloads with new shapes; `message.completed` trajectory events may now exist with **no** corresponding channel envelope (under `publishPolicy: "turn-final"`) — anything assuming 1:1 trajectory↔channel message correspondence breaks (audit: reducers, transcript exports).
...
12. **Cross-context reads become possible**: `readFile`/`listFiles`/`diff`/`log`/`status`/`contextStatus`/`pendingMerge` gain a `context: { contextId }` scope authorized by the ownership tree — a *loosening* of the previous strict own-context isolation (owned contexts only; the boundary gate still rejects everything else). New `vcs.pick` and `vcs.contextDiff` methods; the revert patch-application path is refactored to be direction-agnostic so pick and revert share one engine.
...
- **Lineage-true `forkContext`** (WS-1) is the deepest change: apply forkLog's lineage mechanics (`parent_log_id`/`fork_seq`/`fork_hash`) to ctx heads and copy edit-op rows. There is no fallback design — build it as specified; its integration test (fork dirty → truthful status → divergent commits → merge base → merge) is the anchor of the E3 suite.
- **Recursive context-tree clone** (WS-3) must re-home pending calls across every cloned pair via the same reconciliation `postClone` runs; calls that cannot be re-homed settle as `aborted-by-fork` — never silently dropped.
