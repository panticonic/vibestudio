...
## Landed structured-delivery amendment (2026-07-25)

The generic channel, unified-log, policy, and journal-before-dispatch contracts
below remain binding. Structured delivery no longer calls
`onChannelEnvelope`, uses an in-memory delivery chain, or drains recipients
from `PubSubChannel.alarm()`.

- A structured publish commits `channel_delivery_queue` with the recipient and
  exact subscription incarnation, then emits a disposable
  `channel-delivery` work-ready receipt.
- The host `DurableWorkDriver` claims one ordered batch per
  participant/incarnation lane and calls `acceptChannelBatch` outside channel
  request and alarm lifetimes.
- Settlement requires an exact, complete, gap-free acknowledgement matching
  the worker and lease generation.
- Departure or incarnation replacement terminalizes queued rows for that
  address. A slow recipient retains only its own backlog.
- The channel alarm may expose expired delivery leases and retry-at rows as
  ready. It never invokes a participant or performs stale pending-call
  redelivery.
- RPC subscription-stream fan-out remains the path for panels and
  connectionless RPC-style DO clients, not a second agent delivery route.

Read later `queueDoEnvelope`, `onChannelEnvelope`, structured-delivery alarm,
and stale-redelivery descriptions as historical inventory.
