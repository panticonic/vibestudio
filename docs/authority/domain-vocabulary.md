...
1. **Three levels, one direction.** Concrete operations (wire methods +
   ...
   operations. Information flows _up_ for display only.
2. **The domain is never a grant key.** No _positive_ authority — grant,
...
- **`sharing` is the highest-anxiety domain** and is deliberately separated
  ...
  _leaving_ the workspace (publish, send, open externally, webhook out,
...
- **`safety` exists to be visibly locked.** Capabilities in this domain
  ...
  _see_ that agents structurally cannot grant themselves permissions. This is
...
A **cell** is a (domain, verb) pair — e.g. _Publishing & sending / Do_. The
...
| `mobile.provision`                  | manage | install and pair a phone or tablet     |
...
attempting operations; the _direct_ request-a-prompt capability remains
...
4. **A categorization lie cannot manufacture egress.** A service that
   ...
   `sharing`/network capabilities in its _own_ manifest, which its own
