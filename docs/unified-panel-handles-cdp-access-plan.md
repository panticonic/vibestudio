...
Vibestudio is an Electron tree-browser: the UI is a tree of "panels" (each an Electron
...
- **Client** (`workspace/packages/runtime/src/panel/browserAutomation.ts`): drop the `kind`
  ...
  broker, so panels, workers, and DOs all use one path. (The `globalThis.__vibestudioShell` IPC
