...
1. **Room-per-pairing-invite.** `auth.createPairingInvite` mints a fresh room UUID
   per invite (instead of reusing the singleton `webrtcPairing`); the deep link
   carries it. On redemption, persist `deviceId → room` in the device auth store.
   On server start, arm one answerer pipe per paired device room + per outstanding
   invite; new invites arm a pipe on demand.
...
3. **Relay alarm (plan §12, "not optional"):** answerer records
   ...
   visible.
4. **Native e2e in CI:** run the `VIBEZ1_RUN_WEBRTC_E2E=1` suite on a schedule
