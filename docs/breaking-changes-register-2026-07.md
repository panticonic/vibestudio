...
44. Policy widenings for the eviction: `blobstore` read/write, broad `refs` reads, the `vcs` service, and DO `vcsLog`/`ingestWorktreeState`/`vcsImportPublish` admit `extension` callers. State listings come only from `blobstore.listTree`; the duplicate DO listing RPC was deleted. `refs.updateMains` remains DO-only at the public policy layer and is narrowed further by the invocation-token check.
...
57. Runtime `git` is now exactly the typed `gitInterop` service client. The redundant `git.http` alias, `configureUpstream`, single-row status adapter, split-argument publish overload, broad handwritten result bags, duplicate top-level `importProject.branch`, and direct userland Git Bridge invocation path are deleted. The low-level `@vibestudio/git` client likewise accepts only object inputs for `commit` and `createBranch`; their positional overloads are removed. Callers use `credentials.gitHttp`, `git.setUpstream`, array-based status results, one strict `publishRepo(input)` object, and `importProject.remote.branch`; no compatibility aliases or fallbacks remain.
58. Extension providers now live in exact manifest-declared namespaces rather than the flat public extension API. ABI v3 stores those contracts in approved build metadata and requires the activation/ready surfaces to match; `extensions.invokeProvider` dispatches only to the dedicated child provider handler, with no fallback to flat `extensions.invoke`. Browser Data and Claude callers use their provider slots without resolving extension packages. The complete `gitInterop` contract is host-owned and schema-validated, so generic `extensions.invoke`, `extensions.invokeStream`, and public `extensions.invokeProvider` cannot reach its methods; the typed `gitInterop` service is the only userland route. The `streamingMethods` service result is now strictly an array, with nullable host compatibility removed. Optional operation arguments use shorter tuples instead of `undefined`/`null` transport slots, `upstreamStatus` requires an array (`[]` means all), and `setAutoPush` requires an explicit boolean. Workspace Git remote declarations are object-only; string shorthand, nullable branches, sparse tombstones, and empty map shells are rejected or pruned rather than normalized. Upstream engine state is strict configuration-scoped version 2, while bridge marker/checkout state uses strict versioned envelopes; old, unversioned, and malformed records are discarded without migration. Each operation uses an immutable URL and fingerprint-specific transport ref, and state/runtime writes are fingerprint compare-and-set. Git config edits are narrow transactions against current protected `meta/main`, not stale full snapshots. Git export requires the canonical content tree, so the VCS state-file fallback and its extension permission are removed.
59. `handle.cdp.screenshot(options)` is the sole panel screenshot API and now returns `{ data, mimeType, width, height }` through the visibility-independent host capture path. The lightweight-page raw-byte behavior and temporary `captureScreenshot` alias are removed rather than retained as compatibility surfaces.
60. Git declarations and low-level client writes are object-only. `git.remotes` accepts only `{ url, branch? }` declarations (no URL shorthand, nullable branches, or null/undefined tombstones), config parsing rejects every former shape, removal prunes empty nesting, and `gitInterop` mutations always return concrete canonical maps. `GitClient.commit` and `GitClient.createBranch` accept only option objects. Bridge markers and checkout maps use exact versioned envelopes; unversioned, wrong-version, and malformed records are discarded without migration.
