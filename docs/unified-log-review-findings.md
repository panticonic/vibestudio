...
vibestudio.yml, the worker-pinning API and its docs, persisted worker instances
...
- **Where:** `getBuild` ref validation `src/server/buildV2/index.ts:379-389`
  ...
  - (b) `appHost.ts:1329` and `packages/extension-host/src/service.ts:1319` —
    `decl.ref` from vibestudio.yml (any non-empty string; default "main") goes
...
- **Fix:** decide: implement the vcs-side answer (subscribe to
  `state-advanced(main)` for `projects/vibestudio` paths + gitBridge export →
