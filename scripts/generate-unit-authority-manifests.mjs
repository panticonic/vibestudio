...
// Runtime-owned extension protocol is one product contract shared with the
// build sealer. Source manifests expose the same effective authority for
// review, while the sealer remains correct for newly created extensions before
// this generator has rewritten their package.json.
const extensionRuntimeAuthority = JSON.parse(
  fs.readFileSync(path.join(root, "packages/shared/src/extensionRuntimeAuthority.json"), "utf8")
);
const EXTENSION_RUNTIME_BASE_CAPABILITIES = Object.freeze(extensionRuntimeAuthority.capabilities);
...
const evalDelegationCapabilities = [
  ...new Set([
    ...authorityLedger.rows
      .filter(
    ...
    ...[...(workspaceManifest.services ?? []), ...PRODUCT_WORKSPACE_SERVICES].map(
      (service) => `workspace-service:${service.name}`
    ),
  ]),
].sort();
...
for (const pkg of [...packages.values()].sort((a, b) => a.file.localeCompare(b.file))) {
  ...
  const delegations = pkg.manifest.vibestudio?.agent
    ...
    : (pkg.manifest.vibestudio?.authority?.delegations ?? []);
