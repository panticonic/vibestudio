...
function ensureBrandedMacElectronApp(electronExecutable, opts = {}) {
  ...
  const bundleIdentifier = installed ? "app.vibestudio.app" : APP_BUNDLE_IDENTIFIER;
  ...
  const cacheBase = installed
    ? path.join(os.homedir(), "Library", "Application Support", "vibestudio", "electron-cache")
    : path.join(repoRoot, ".cache", "vibestudio-electron");
