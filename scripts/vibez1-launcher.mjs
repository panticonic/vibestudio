#!/usr/bin/env node
// Unified `vibez1` entry point for the npm-published packages.
//
// - @vibez1/app ships this with Electron present: no args / `open` → launch
//   the desktop GUI; a recognized verb (remote, pair, mobile, fs, vcs, agent,
//   eval, --help, --version, ...) → delegate to the CLI client.
// - @vibez1/server ships the same file but without Electron: every invocation
//   routes to the CLI (bare invocation prints CLI help).
//
// Both variants pin VIBEZ1_APP_ROOT to the installed package so getAppRoot()
...
if (!process.env["VIBEZ1_APP_ROOT"]) {
  process.env["VIBEZ1_APP_ROOT"] = packageRoot;
}
...
async function launchGui(args) {
  ...
  const { resolveElectronExecutableForVibez1 } = await import(
    pathToFileURL(path.join(packageRoot, "scripts", "branded-electron.mjs")).href
  );
  const electronBinary = resolveElectronExecutableForVibez1({ installed: true });
  ...
  const child = spawn(electronBinary, [packageRoot, ...args], {
    ...
    env: {
      ...
      ...(packageName() ? { VIBEZ1_NPM_CHANNEL: packageName() } : {}),
    },
  });
  const cleanupSignals = forwardSignals(child);
  child.on("exit", (code, signal) => endWith(cleanupSignals, code, signal));
