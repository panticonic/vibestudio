...
remote-serve hub, consumes the fresh mobile root invite from the strict
ready-file contract, and sends its compact Iroh endpoint, relay, and pairing
material in a vibestudio://connect intent to the launched internal app, confirms the Pair
...
Remote reach is the encrypted Iroh transport using the production public-relay
defaults (no Tailscale or legacy peer-signaling infrastructure).
`);
}
...
// Parse the Iroh pairing link and rebuild it with the canonical deep-link
// carrier. Pairing material is identical across the HTTPS and app-scheme forms.
...
// service-bind retries) — long enough to make any transport appear dead during
// panel load. The harness was producing the
...
async function main() {
  ...
  try {
    ...
    // 1. Start the normal remote-serve hub with the public-relay defaults.
    ...
    // Embedded Iroh flow. Phases are emitted by apps/mobile/index.js.
    const phases = [
    ...
    console.log(
      `[mobile-smoke] Iroh:      endpoint=${parsedLink.endpointId}; relays=${parsedLink.relays.join(",")}`
    );
    ...
    if (
      ...
      throw new Error("Warm app relaunch fetched panel artifact bytes over the Iroh pipe");
    }
    console.log(
