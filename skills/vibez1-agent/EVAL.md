...
`vibez1 eval run` executes TypeScript/JavaScript **server-side** in your CLI
...
| `rpc.callTarget(targetId, method, args)` | Call a runtime entity (DO/worker) by target id, e.g. after `workers.resolveService`: `const svc = await rpc.call("workers.resolveService", ["vibez1.testkit-driver.v1", null]); await rpc.callTarget(svc.targetId, "ping", [])` |
...
- `--fresh-scope` resets the persistent scope (and the user `db`) before the
  run, so the snippet starts empty; `vibez1 eval repl-reset` does the same
  reset as a standalone command.

## Imports

```bash
vibez1 eval run --imports '{"lodash":"npm:4","@workspace/gad":"latest"}' -e '
