...
```bash
vibez1 remote pair "vibez1://connect?url=https://host.ts.net&code=ABC123"
vibez1 remote status                      # verify credential + reachability
vibez1 agent attach                       # session "default"
vibez1 agent services | head              # what can I call?
vibez1 agent skills                       # what does this workspace document?
```

One-shot variant — when no credential is stored yet, attach can pair first
(if already paired this is a usage error; `vibez1 remote logout` to re-pair):

```bash
vibez1 agent attach work --url https://host.ts.net --code ABC123
```
...
3. **`vibez1 vcs push`** advances `main`, **fast-forward-only** and
   **build-gated**. A push that comes back `build-failed` did NOT advance
   `main` — the diagnostics it prints are your next task list.

```bash
vibez1 fs ls /                                        # context root: panels/, workers/, ...
vibez1 fs grep "registerPanel" panels/notes -C 2
vibez1 fs read panels/notes/src/index.tsx > /tmp/index.tsx
# ...edit locally...
vibez1 fs write panels/notes/src/index.tsx --from-file /tmp/index.tsx   # WORKING edit
vibez1 vcs status --repo panels/notes                # changed paths + uncommitted count
vibez1 agent call vcs.commit '[{"message":"Fix panel registration"}]'  # seal the edits
vibez1 vcs push   --repo panels/notes                # advance main (build-gated)
```
...
```bash
vibez1 fs read panels/notes/src/index.tsx | sed -n '40,44p'   # inspect line 42
# ...fix the type at index.tsx:42 and the import at :58 (fs write = vcs.edit)...
vibez1 agent call vcs.commit '[{"message":"Fix types + import"}]'   # seal the fix
vibez1 vcs push --repo panels/notes                                # re-push
```
...
> `vibez1 agent call vcs.previewBuild '[{"repoPaths":["panels/notes"]}]'`.

Drive the loop from a script with `--json` (the full `VcsPushResult`):

```bash
result=$(vibez1 vcs push --repo panels/notes --json) || {
...
vibez1 fs write panels/mynote/index.tsx \
  --content 'export default function MyNote() { return <div>hi</div>; }'
vibez1 fs write panels/mynote/package.json --content '{
  "name": "@workspace-panels/mynote",
  "vibez1": { "title": "My Note" },
  "dependencies": { "@workspace/runtime": "workspace:*", "@workspace/react": "workspace:*" }
}'
vibez1 agent call vcs.commit '[{"message":"Create mynote panel"}]'   # seal the new files
vibez1 vcs push --repo panels/mynote                                 # creates main from empty
vibez1 vcs log  --repo panels/mynote                                 # one entry: the first commit
```

A new **package** is the same shape:

```bash
vibez1 fs write packages/mylib/index.ts --content 'export const add = (a:number,b:number)=>a+b;'
vibez1 fs write packages/mylib/package.json --content '{ "name": "@workspace/mylib", "exports": { ".": "./index.ts" } }'
vibez1 agent call vcs.commit '[{"message":"Add mylib package"}]'
vibez1 vcs push --repo packages/mylib
```
...
```bash
vibez1 vcs fork-repo panels/chat panels/mychat   # FROM_REPO TO_REPO; inherits history
vibez1 vcs log --repo panels/mychat              # shows the inherited commits
# The package.json name leaf is already rewritten. Make the DEEPER renames yourself:
vibez1 fs grep -i "chat" panels/mychat           # find component/class/contract names to rename
# ...rename them with fs write (= vcs.edit), then commit and push the fork:
vibez1 agent call vcs.commit '[{"message":"Fork chat → mychat"}]'
vibez1 vcs push --repo panels/mychat
```
...
vibez1 agent call vcs.commit '[{"message":"Rename Button prop"}]'   # commits all edited repos
vibez1 vcs push --repo packages/ui --repo panels/notes
```
...
- **`mergeable:"clean"`** — no overlapping changes; the merge already committed.
  Just re-push: `vibez1 vcs push --repo panels/notes`.
...
```bash
vibez1 agent call vcs.merge '["panels/notes"]' | jq -r '.conflictPaths[]'
vibez1 fs read panels/notes/src/index.tsx        # contains <<<<<<< / ======= / >>>>>>> markers
# ...edit out the markers (fs write = vcs.edit)...
vibez1 fs write panels/notes/src/index.tsx --from-file /tmp/resolved.tsx
vibez1 agent call vcs.commit '[{"message":"Merge main into notes"}]'   # seals the resolution
vibez1 vcs push --repo panels/notes                                    # now fast-forwards
```

To abandon uncommitted edits (and any pending merge) on a repo, drop them:

```bash
vibez1 agent call vcs.discardEdits '["panels/notes"]'
```
...
vibez1 agent call vcs.fileHistory '["panels/notes","src/index.tsx"]'

# The exact edit-ops a commit folded in (commit event id from vcs.log).
vibez1 agent call vcs.commitEdits '["panels/notes",{"eventId":"evt-123"}]'

# Walk a commit's ancestry in the event-keyed commit DAG.
vibez1 agent call vcs.commitAncestors '["panels/notes","evt-123"]'
```
...
```bash
vibez1 vcs push-status --repo panels/notes    # how far ahead of main am I?
vibez1 vcs diff        --repo panels/notes     # name-status of unpushed changes
vibez1 vcs log         --repo panels/notes --limit 10
vibez1 vcs log         --repo meta             # config history (meta is a content repo)
```
...
vibez1 eval run -e 'return scope.entities.filter(e => e.kind === "panel").map(e => e.id)'
vibez1 eval repl-reset      # when the cached state goes stale
```
...
```bash
vibez1 agent sessions | jq -r '.[].name'
vibez1 fs grep "TODO" --max 50 | jq '.matchCount'
vibez1 vcs status --repo panels/notes | jq '{uncommitted, added, changed, removed}'
vibez1 vcs push --repo panels/notes --json | jq -r '.status'
```

## Call a service the CLI has no command for

```bash
vibez1 agent services workspace --json | jq '.methods | keys'   # check the schema
vibez1 agent call workspace.listSkills '[]'
vibez1 agent call vcs.status "[\"panels/notes\",\"ctx:$(vibez1 agent status --json | jq -r .contextId)\"]"
```
...
```bash
vibez1 agent call runtime.createEntity '[{"kind":"worker","source":"workers/stats","key":"stats-1"}]'
vibez1 agent call ping --target "worker:workers/stats:stats-1"   # relayed: plain method name
vibez1 agent call runtime.retireEntity '[{"id":"worker:workers/stats:stats-1"}]'
```
...
```bash
vibez1 agent call workspace.units.list '[]'
vibez1 agent logs my-worker --level warn --limit 100
vibez1 eval run -e 'return await help("workers")'
```
...
EOF
vibez1 eval run /tmp/report.ts --imports '{"lodash":"npm:4"}'
```
...
```bash
vibez1 agent attach featureA
vibez1 agent attach bugfixB
vibez1 fs write notes.md --content "task A" --session featureA
vibez1 fs ls / --session bugfixB        # does not see featureA's files
vibez1 agent detach featureA --rm       # clean up: retire + delete context
```

## Invite another device

```bash
vibez1 remote invite --ttl-ms 600000    # prints a pairing code + vibez1:// link
```

## Install this skill into a project

```bash
vibez1 agent skill install              # -> ./.claude/skills/vibez1-agent
vibez1 agent skill install --dir ~/myproj/.claude/skills/vibez1-agent
```
