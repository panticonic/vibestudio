...
All `vibez1 fs` and `vibez1 vcs` commands operate inside the **context
folder** of an attached agent session on the server. Pick the session with
`--session NAME` (default `default`); attach it first with
`vibez1 agent attach [NAME]`. Remote paths are POSIX-style and relative to
the context root (`/` = context root). All commands accept `--json`.

## Sessions

```bash
vibez1 agent attach [NAME] [--url U --code C]   # create or reuse; --url/--code pair first (errors if already paired)
vibez1 agent status [NAME]                      # verify the entity is live (exit 5 if stale)
vibez1 agent sessions                           # list local sessions, live/stale
vibez1 agent detach [NAME] [--rm]               # retire entity; --rm also deletes the context folder
```

Session records live locally at `~/.config/vibez1/agent-sessions/<name>.json`;
the device credential at `~/.config/vibez1/cli-credentials.json` (both 0600).

## fs commands

```bash
vibez1 fs ls [PATH] [-R]                  # list directory; dirs print with trailing "/"; -R recurses
vibez1 fs read PATH [--out FILE]          # raw bytes to stdout (binary-safe), or save to a local FILE
vibez1 fs write PATH [CONTENT] [--content TEXT | --from-file F] [--append]
                                            # content from positional, flag, local file, or stdin; creates parents
vibez1 fs rm PATH [-r]                    # -r for directories
vibez1 fs mv SRC DEST                     # rename/move
vibez1 fs cp SRC DEST                     # copy a file
vibez1 fs mkdir PATH [-p]                 # -p creates parents
vibez1 fs stat PATH                       # size/mtime/type (JSON)
vibez1 fs grep PATTERN [PATH] [-i] [--glob G] [-C N] [--max N]
vibez1 fs glob PATTERN [PATH]             # find files by glob, newest first
```
...
2. **commit** — `vcs.commit(message)` folds your uncommitted edits into one
   ...
   (`vcs.commitEdits`).
3. **push** — `vibez1 vcs push --repo <p>` is the **only** way to advance
...
vibez1 fs write panels/notes/src/index.tsx --from-file /tmp/index.tsx

# commit those working edits into a messaged snapshot (RPC — no dedicated command)
vibez1 agent call vcs.commit '[{"message":"Fix panel registration"}]'

# push the committed snapshot into main — build-gated, fast-forward-only
vibez1 vcs push --repo panels/notes

vibez1 vcs status      --repo panels/notes              # this repo's unpushed changes + uncommitted count
vibez1 vcs diff        --repo panels/notes              # name-status diff of those changes
vibez1 vcs push-status --repo panels/notes              # how far ahead of main + uncommitted/diverged (pre-push)
vibez1 vcs log         --repo panels/notes [--limit N]  # this repo's COMMIT history (edits never appear)
vibez1 vcs fork-repo   panels/chat panels/mychat        # copy a repo to a new path, keeping history
```
...
edit and commit layers are RPCs via `vibez1 agent call` (and `fs write` is the
...
vibez1 agent call vcs.commit '[{"message":"Create mynote panel"}]'

# 3. Push to create the repo's main from empty, build-gated.
vibez1 vcs push --repo panels/mynote
```
...
`vcs.commit`, then `vibez1 vcs push --repo packages/mylib`.
...
```bash
vibez1 vcs fork-repo panels/chat panels/mychat   # FROM_REPO  TO_REPO
```
...
```bash
vibez1 agent call SERVICE.METHOD 'ARGS_JSON' [--target ID]
```

`ARGS_JSON` is a JSON **array** of positional arguments. `fs.*` take
the session's contextId as their first argument (get it from
`vibez1 agent status --json`). The per-repo VCS methods have these exact arg
...
`vibez1 agent call ping --target worker:...`). See
[API.md](API.md) for the service list and
`vibez1 agent services NAME --json` for full schemas.

## Other agent commands

```bash
vibez1 agent services [NAME]    # list RPC services, or describe one (full Zod schemas with --json)
vibez1 agent skills [NAME]      # list workspace skills, or print one SKILL.md
vibez1 agent logs UNIT [--since MS] [--level L] [--limit N]   # workspace unit logs
vibez1 agent skill install [--dir DIR] | print   # install this skill (default ./.claude/skills/vibez1-agent)
```
