...
`vibez1 vcs push` is the one way to advance a repo's `main`. It is
...
1. **Content-only repos are ungated.** A repo that resolves to no build unit —
   ...
   `meta/vibez1.yml` advances `vcs:repo:meta` with history but never builds.)
...
A **fork** (`vibez1 vcs fork-repo FROM TO`) is the other way to get a new
...
```bash
vibez1 agent call vcs.commit '[{"message":"Rename Button prop"}]'   # commits both edited repos
...
```bash
vibez1 vcs push --repo packages/ui --json | jq -r '
...
- `vibez1 agent diag UNIT` — runtime errors/logs + recent build events for a
  running unit.
- `vibez1 agent logs UNIT [--level error]` — the log tail.
