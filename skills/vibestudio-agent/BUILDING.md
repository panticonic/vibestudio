...
`vibestudio vcs push` is the one way to advance a repo's `main`. It is
...
1. **Content-only repos are ungated.** A repo that resolves to no build unit —
   ...
   `meta/vibestudio.yml` advances `vcs:repo:meta` with history but never builds.)
...
A **fork** (`vibestudio vcs fork-repo FROM TO`) is the other way to get a new
...
```bash
vibestudio agent call vcs.commit '[{"message":"Rename Button prop"}]'   # commits both edited repos
...
```bash
vibestudio vcs push --repo packages/ui --json | jq -r '
...
- `vibestudio agent diag UNIT` — runtime errors/logs + recent build events for a
  running unit.
- `vibestudio agent logs UNIT [--level error]` — the log tail.
