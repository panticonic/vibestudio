...
| `gitInterop.upstreamStatus` | Return external Git upstream status for tracked repos, including integration-required candidate coordinates. Arguments are positional: call `git.upstreamStatus([imported.path], { fetch: false })`, not `git.upstreamStatus([[imported.path], { fetch: false }])`. The configured gitInterop provider performs any Git/network work. |
