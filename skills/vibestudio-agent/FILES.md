...
All `vibestudio fs` and `vibestudio vcs` commands operate inside the **context
folder** of an attached agent session on the server. Pick the session with
`--session NAME` (default `default`); attach it first with
`vibestudio agent attach [NAME]`. Remote paths are POSIX-style and relative to
...
Session records live locally at `~/.config/vibestudio/agent-sessions/<name>.json`;
the device credential at `~/.config/vibestudio/cli-credentials.json` (both 0600).

## fs commands

```bash
vibestudio fs ls [PATH] [-R]                  # list directory; dirs print with trailing "/"; -R recurses
vibestudio fs read PATH [--out FILE]          # raw bytes to stdout (binary-safe), or save to a local FILE
vibestudio fs write PATH [CONTENT] [--content TEXT | --from-file F] [--append]
                                            # content from positional, flag, local file, or stdin; creates parents
vibestudio fs rm PATH [-r]                    # -r for directories
vibestudio fs mv SRC DEST                     # rename/move
vibestudio fs cp SRC DEST                     # copy a file
vibestudio fs mkdir PATH [-p]                 # -p creates parents
vibestudio fs stat PATH                       # size/mtime/type (JSON)
vibestudio fs grep PATTERN [PATH] [-i] [--glob G] [-C N] [--max N]
vibestudio fs glob PATTERN [PATH]             # find files by glob, newest first
```
...
vibestudio agent call vcs.commit '[{"message":"Fix panel registration"}]'

# push the committed snapshot into main — build-gated, fast-forward-only
vibestudio vcs push --repo panels/notes

vibestudio vcs status      --repo panels/notes              # this repo's unpushed changes + uncommitted count
vibestudio vcs diff        --repo panels/notes              # name-status diff of those changes
vibestudio vcs push-status --repo panels/notes              # how far ahead of main + uncommitted/diverged (pre-push)
vibestudio vcs log         --repo panels/notes [--limit N]  # this repo's COMMIT history (edits never appear)
vibestudio vcs fork-repo   panels/chat panels/mychat        # copy a repo to a new path, keeping history
```
...
vibestudio agent call vcs.commit '[{"message":"Create mynote panel"}]'

# 3. Push to create the repo's main from empty, build-gated.
vibestudio vcs push --repo panels/mynote
```
...
`vcs.commit`, then `vibestudio vcs push --repo packages/mylib`.
...
```bash
vibestudio vcs fork-repo panels/chat panels/mychat   # FROM_REPO  TO_REPO
```
...
`vibestudio agent call ping --target worker:...`). See
[API.md](API.md) for the service list and
`vibestudio agent services NAME --json` for full schemas.
