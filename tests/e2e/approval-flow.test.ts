...
} from "../../src/server/services/approvalPushBridge.js";
import {
  createApprovalQueue,
  type ApprovalQueueWithListeners,
} from "../../src/server/services/approvalQueue.js";
import { createPushMetrics, type PushMetrics } from "../../src/server/services/pushMetrics.js";
import {
  createPushService,
  type PushServiceResult,
} from "../../src/server/services/pushService.js";
...
class FakeTransport {
  constructor(
    ...
    // Both shells belong to the same member (family/team model): push
    // registrations attribute to this subject so a resolution on one device
    // fans a cancel out to the user's other devices.
    private readonly userId: string = "u-member"
  ) {}
  async call(_target: "main", rpcMethod: string, args: unknown[]): Promise<unknown> {
    ...
    return service.handler(
      {
        caller: createVerifiedCaller(this.callerId, "shell", null, null, {
          userId: this.userId,
          handle: "member",
        }),
      },
...
describe("approval flow e2e", () => {
  ...
  it("settles only the first decision when mobile and desktop resolve at the same time", async () => {
    ...
    const resolutions = await Promise.allSettled([
      ...
      harness.desktopTransport.call("main", RPC_METHODS.shellApproval.resolve, [
        approvalId,
        "deny",
      ]),
    ]);
    expect(resolutions.map((resolution) => resolution.status)).toEqual(["fulfilled", "rejected"]);
    expect(resolutions[1]).toMatchObject({
      reason: expect.objectContaining({
        message: expect.stringContaining("already being resolved"),
      }),
    });
    await expect(approvalPromise).resolves.toBe("once");
    expect(harness.approvalQueue.listPending()).toEqual([]);
  ...
  it("fans out a cancel push when another shell resolves a sent mobile approval", async () => {
    ...
    await flushAsyncWork();
    const approvalId = harness.approvalQueue.listPending()[0]!.approvalId;
    await harness.desktopTransport.call("main", RPC_METHODS.shellApproval.resolve, [
      approvalId,
      "session",
    ]);
