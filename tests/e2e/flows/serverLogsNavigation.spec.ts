...
function configureInitialPanel(workspacePath: string): void {
...
test.describe("Server Logs navigation", () => {
  test("opens from About/New without leaving a stuck pending spinner", async () => {
    ...
    configureInitialPanel(workspacePath);
      ...
      await expect
        .poll(
          async () => {
            ...
            return panel?.snapshot?.source === "about/server-logs" && (await isPanelReady(testApp!.app, panelId));
          },
