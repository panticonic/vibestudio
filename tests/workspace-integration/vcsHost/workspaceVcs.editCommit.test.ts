...
import type { StateAdvancedEvent, WorkingAdvancedEvent } from "../../../src/server/buildV2/stateTrigger.js";
...
describe("WorkspaceVcs edit → commit → push", () => {
  ...
  it("edit records uncommitted ops without advancing the commit head, log, or state-advanced", async () => {
    ...
    expect(await gad.instance.vcsLog(REPO, 50, CTX_HEAD)).toHaveLength(0);
  ...
  it("commit folds edits into a snapshot, claims them (queryable both ways), requires a message", async () => {
    ...
    const edits = await gad.instance.vcsCommitEdits(REPO, result.eventId!);
  ...
  it("push rejects on uncommitted edits", async () => {
    ...
    await expect(
      pushToMain(gad, { repoPaths: [REPO], sourceHead: CTX_HEAD, actor: AGENT })
  ...
  it("push rejects with a structured divergence error; vcs.merge reconciles → push fast-forwards", async () => {
    ...
    expect((await pushToMain(gad, { repoPaths: [REPO], sourceHead: otherCtx, actor: USER })).status).toBe(
      "pushed"
    );
  ...
  it("vcs.merge conflict parks a pending merge; resolve via edit → commit seals it", async () => {
    ...
    await pushToMain(gad, { repoPaths: [REPO], sourceHead: otherCtx, actor: USER });
  ...
  it("fileHistory returns a path's edits in commit-lineage order", async () => {
    ...
    const mainHistory = await gad.instance.vcsFileHistory(REPO, "a.txt", VCS_MAIN_HEAD);
    // Narrow-host P3: the DO's push records main-advance provenance with per-file
    // editOps AND links the source commit as first parent (handoff A2), so main's
    // a.txt lineage carries the source `create` plus the push commit — richer than
    // the pre-flip squashed single main commit. Both are committed, newest first.
    expect(mainHistory).toHaveLength(2);
    expect(mainHistory.every((h) => h.committedEventId !== null)).toBe(true);
    expect(mainHistory[0]!.newContentHash).toBeTruthy();

    const ctxHistory = await gad.instance.vcsFileHistory(REPO, "a.txt", CTX_HEAD);
  ...
  it("links edits to their authoring agent invocation (preserved through commit; turn via traversal)", async () => {
    ...
    const claimed = await gad.instance.vcsCommitEdits(REPO, committed.eventId!);
  ...
  it("commitAncestors keys on event_id — distinguishes two commits with an identical state", async () => {
    ...
    const ancA = await gad.instance.vcsCommitAncestors(REPO, cA.eventId!);
    const ancB = await gad.instance.vcsCommitAncestors(REPO, cB.eventId!);
    ...
    const ancA2 = await gad.instance.vcsCommitAncestors(REPO, cA2.eventId!);
