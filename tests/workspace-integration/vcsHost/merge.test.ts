...
describe("WorkspaceVcs merge", () => {
  ...
  async function seedMain(
    ...
    const pushed = await pushToMain(gad, { repoPaths: [REPO], sourceHead: seedHead, actor: USER });
  ...
  async function advanceMain(
    ...
    const pushed = await pushToMain(gad, { repoPaths: [REPO], sourceHead: otherCtx, actor: USER });
    expect(pushed.status).toBe("pushed");
  ...
  it("surfaces non-content conflicts (delete-vs-change)", async () => {
    ...
    const ancestors = await gad.instance.vcsCommitAncestors(REPO, sealed.eventId!);
  ...
  it("re-materializes a pending merge whose markers never hit the worktree", async () => {
    ...
    const spy = vi
      .spyOn(vcs.worktrees, "materializeState")
...
describe("WorkspaceVcs gc", () => {
  ...
  it("keeps referenced history, sweeps staged orphans and dead blobs", async () => {
    ...
    const { putBytes } = await import("../../../src/server/services/blobstoreService.js");
    ...
    const { blobPath } = await import("../../../src/server/services/blobstoreService.js");
  ...
  it("keeps content-store-only trees reachable from protected refs", async () => {
    const { blobPath, mirrorWorktreeTree, putBytes } =
      await import("../../../src/server/services/blobstoreService.js");
    const blobsDir = path.join(root, "blobs");
    const liveBytes = Buffer.from("protected content-store root\n");
    const { digest: liveDigest } = await putBytes(blobsDir, liveBytes);
    const liveTree = await mirrorWorktreeTree(blobsDir, [
      { path: "live.txt", contentHash: liveDigest, mode: 33188 },
    ]);
    const stateNodeDigest = treeHashDigest(liveTree.stateHash);
    const rootTreeDigest = treeHashDigest(liveTree.treeHash);

    // Simulate a crash window where the protected ref is authoritative and the
    // content-store tree is live, but the DO has no worktree-state rows for it.
    await refs.seedMain({ repoPath: REPO, value: liveTree.stateHash });
    for (const digest of [liveDigest, stateNodeDigest, rootTreeDigest]) {
      gad.instance.ensureBlob(digest, 1);
    }
    const pastGrace = new Date(Date.now() - 20 * 60 * 1000).toISOString();
    gad.sql.exec(`UPDATE gad_blobs SET created_at = ?`, pastGrace);

    const result = await vcs.runGc({ minAgeMs: 0 });
    expect(result.sweptBlobs).toBe(0);
    for (const digest of [liveDigest, stateNodeDigest, rootTreeDigest]) {
      await expect(fsp.access(blobPath(blobsDir, digest))).resolves.toBeUndefined();
    }
    const file = await vcs.readFile(VCS_MAIN_HEAD, "live.txt", REPO);
    expect(file?.content).toEqual({ kind: "text", text: "protected content-store root\n" });
  });

  it("sweeps orphaned content-store tree nodes with no DO rows", async () => {
    const { blobPath, mirrorWorktreeTree, putBytes } =
      await import("../../../src/server/services/blobstoreService.js");
    const blobsDir = path.join(root, "blobs");

    const { digest: liveDigest } = await putBytes(blobsDir, Buffer.from("live tree\n"));
    const liveTree = await mirrorWorktreeTree(blobsDir, [
      { path: "live.txt", contentHash: liveDigest, mode: 33188 },
    ]);
    await refs.seedMain({ repoPath: REPO, value: liveTree.stateHash });

    const { digest: orphanDigest } = await putBytes(blobsDir, Buffer.from("orphan tree\n"));
    const orphanTree = await mirrorWorktreeTree(blobsDir, [
      { path: "orphan.txt", contentHash: orphanDigest, mode: 33188 },
    ]);
    const orphanStateDigest = treeHashDigest(orphanTree.stateHash);
    const orphanRootDigest = treeHashDigest(orphanTree.treeHash);

    const result = await vcs.runGc({ minAgeMs: 0 });

    expect(result.sweptTreeObjects).toBeGreaterThanOrEqual(2);
    await expect(
      fsp.access(blobPath(blobsDir, treeHashDigest(liveTree.stateHash)))
    ).resolves.toBeUndefined();
    await expect(
      fsp.access(blobPath(blobsDir, treeHashDigest(liveTree.treeHash)))
    ).resolves.toBeUndefined();
    await expect(fsp.access(blobPath(blobsDir, orphanStateDigest))).rejects.toThrow();
    await expect(fsp.access(blobPath(blobsDir, orphanRootDigest))).rejects.toThrow();
    await expect(fsp.access(blobPath(blobsDir, orphanDigest))).resolves.toBeUndefined();
  });
});
