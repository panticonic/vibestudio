...
describe("WorkspaceVcs.ensureRepoLogsFromDisk (disk bootstrap)", () => {
  ...
  it("seed-on-attach is idempotent across restart and never moves an advanced ref", async () => {
    ...
    expect(after.map((record) => record.repoPath).sort()).toEqual(
  ...
  it("contextStatus does not mark an already-pushed stale ctx head as ahead", async () => {
    ...
    expect(
      (await pushToMain(gad, { repoPaths: ["panels/chat"], sourceHead: head, actor: USER })).status
    ).toBe("pushed");
  ...
  it("Tier 4: a brand-new repo created mid-session gets a ctx head + first push creates main", async () => {
    ...
    await pushToMain(gad, { repoPaths: ["panels/brandnew"], sourceHead: head, actor: USER });
