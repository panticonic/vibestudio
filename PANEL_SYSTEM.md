...
Vibestudio panels are dynamically loaded TypeScript apps that run in isolated webviews. Panels navigate to each other via URL-based navigation.

## Panel Structure

```
my-panel/
├── package.json      # Manifest with vibestudio field (required)
...
2. **Otherwise, an explicit `vibestudio.template`** → that template's `index.html` and `template.json` are used.
...
import {
  ...
  notifications,
  webhooks,
  services, // Dynamic caller-scoped service namespace (portable)
  hosts, // Owner-scoped attached-host client (portable)
  runtime, // Typed lifecycle/supervision client (portable)
...
[`skills/sandbox/RUNTIME_API.md` in Base](https://github.com/panticonic/vibestudio-base/blob/main/skills/sandbox/RUNTIME_API.md)
...
`gatewayFetch` is gateway-origin scoped: use a relative route or an absolute
URL on the configured gateway origin. It rejects cross-origin URLs; use
`credentials.fetch` for external HTTP. The injected `fs` uses the same
context-bound filesystem as every runtime target and has no implicit RPC
