...
Runtime dependency layout is separate from the shared `external-deps` cache used during bundling. Build-time dependency borrows install with scripts disabled and remain leased through bundling/typechecking so panels and workers remain safe from both lifecycle scripts and concurrent eviction. Extension builds resolve an extension-specific dependency materialization for the exact dependency identity. That step may run package lifecycle scripts because the user has already granted the extension native-code trust; the completed dependency tree is materialized into the immutable bundle directory as `node_modules`, so activation and rollback do not depend on the shared installation cache remaining present. A failed runtime dependency install leaves the extension in `error` with the install log attached to `lastError`.
