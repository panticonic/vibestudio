...
/**
 * `natstack agent skill install|print` — install (or print) the bundled
 * natstack-agent Claude Code skill that documents this CLI.
 */

const SKILL_NAME = "natstack-agent";
 ...
 * skills/natstack-agent next to client.mjs (dist/cli/skills/natstack-agent).
 * Dev (tsx on src/): falls back to <repoRoot>/skills/natstack-agent.
 * NATSTACK_AGENT_SKILL_DIR overrides (test seam).
 */
export function resolveSkillDir(): string {
  const override = process.env["NATSTACK_AGENT_SKILL_DIR"];
...
async function skill(inv: ParsedInvocation): Promise<number> {
  ...
  try {
    ...
    switch (action) {
      ...
      default:
        throw new UsageError("usage: natstack agent skill install [--dir DIR] | print");
    }
  } catch (error) {
...
export const skillCommand: CliCommand = {
  group: "agent",
  name: "skill",
  summary: "Install or print the bundled natstack-agent Claude Code skill",
  usage: "natstack agent skill install [--dir DIR] | print",
  flags: [
    {
      name: "dir",
      takesValue: true,
