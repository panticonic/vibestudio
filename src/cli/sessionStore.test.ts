...
describe("sessionStore", () => {
  ...
  it("round-trips a session file with 0600 permissions", () => {
    ...
    const filePath = path.join(tmpDir, ".config", "vibestudio", "agent-sessions", "alpha.json");
  ...
  it("returns null for missing or malformed session files", () => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".config", "vibestudio", "agent-sessions"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".config", "vibestudio", "agent-sessions", "broken.json"),
      "{not json"
    );
