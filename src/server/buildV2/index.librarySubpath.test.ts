...
describe("BuildSystemV2 library package subpaths", () => {
  ...
  it("builds a declared sandbox test suite as an exact immutable artifact", async () => {
    ...
    fs.writeFileSync(
      ...
      "export const setCurrentTestFile = (_file: string) => {}; export const test = (_name: string, _fn: () => void) => {}; export const exposeTestRunner = (rpc: { expose: (...args: unknown[]) => unknown }, runtime: string) => rpc.expose('tests.run', () => ({ runtime }), { kind: 'closed', reason: 'test runner' });\n"
    );
    fs.writeFileSync(
  ...
  it.each(["workers", "packages"] as const)(
    "builds %s workerd suites with the same Node compatibility as their runtime",
    async (unitRoot) => {
      const workspaceRuntimeDir = path.join(workspaceRoot, "packages", "runtime");
      const runtimeDir = path.join(workspaceRoot, "packages", "test-runtime");
      const workerDir = path.join(workspaceRoot, unitRoot, "portable");
      fs.mkdirSync(path.join(workspaceRuntimeDir, "src"), { recursive: true });
      fs.mkdirSync(path.join(runtimeDir, "src"), { recursive: true });
      ...
      fs.writeFileSync(
        path.join(runtimeDir, "src", "index.ts"),
        "export const setCurrentTestFile = () => {}; export const test = (_name: string, _fn: () => void) => {}; export const exposeTestRunner = (rpc: { expose: (...args: unknown[]) => unknown }, runtime: string) => rpc.expose('tests.run', () => ({ runtime }), { kind: 'closed', reason: 'test runner' });\n"
      );
      fs.writeFileSync(
        path.join(workerDir, "package.json"),
        JSON.stringify({
          name: unitRoot === "workers" ? "@workspace-workers/portable" : "@workspace/portable",
          type: "module",
          vibestudio: {
            entry: "index.ts",
      ...
      const target = `${unitRoot}/portable`;
      const stateRef = `state:${"a".repeat(64)}`;
      const artifact = await buildSystem.getTestArtifact(target, stateRef);
      expect(artifact).toMatchObject({
        target,
        suite: "unit",
        runtime: "workerd",
      ...
      expect(artifact.execution.executionDigest).toMatch(/^[0-9a-f]{64}$/u);
      const executionBuild = await buildSystem.getBuild(target, stateRef, {
        test: { suite: "unit", runtime: "workerd", include: ["**/*.test.ts"] },
      });
      expect(executionBuild.metadata.kind).toBe("worker");
    }
  );

  it.each(["panels", "packages"] as const)(
    "builds %s browser suites with the same fs and path shims as their runtime",
    async (unitRoot) => {
      const workspaceRuntimeDir = path.join(workspaceRoot, "packages", "runtime");
      const runtimeDir = path.join(workspaceRoot, "packages", "test-runtime");
      const panelDir = path.join(workspaceRoot, unitRoot, "portable");
      fs.mkdirSync(path.join(workspaceRuntimeDir, "src"), { recursive: true });
      fs.mkdirSync(path.join(runtimeDir, "src"), { recursive: true });
      fs.mkdirSync(panelDir, { recursive: true });
      fs.writeFileSync(
      ...
      fs.writeFileSync(
        path.join(panelDir, "package.json"),
        JSON.stringify({
          name: unitRoot === "panels" ? "@workspace-panels/portable" : "@workspace/portable",
          type: "module",
          vibestudio: {
            entry: "index.tsx",
            tests: [{ name: "unit", runtime: "browser", include: ["**/*.test.ts"] }],
      ...
      const executionBuild = await buildSystem.getBuild(target, stateRef, {
        test: { suite: "unit", runtime: "browser", include: ["**/*.test.ts"] },
      });
      expect(executionBuild.metadata.kind).toBe("panel");
    }
  );
});
