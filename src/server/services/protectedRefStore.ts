/**
 * ProtectedRefStore — the host's protected MAIN-ref table, reduced to a semantics-free
...
export interface UpdateMainsInput {
   ...
   * caller an approval prompt needs). ProtectedRefStore never inspects or persists it
   * — advancement policy stays injected.
   */
  gateContext?: unknown;
  /**
   * Movement provenance recorded to the main-ref log (§2). The RPC layer
   * (`refsRpcService`) resolves these before they are discarded: `operation`/
  ...
  const service: ProtectedRefStore = {
    ...
    async updateMains(input) {
      ...
      const runBatch = async (): Promise<UpdateMainsResult> => {
        ...
        //    a removal from `next === null`; ProtectedRefStore supplies no semantics.
        await deps.gate({
      ...
      if (changes.length > 0 && refsChangedListeners.size > 0) {
        for (const listener of refsChangedListeners) {
          try {
            ...
            console.error("[protectedRefStore] onRefsChanged listener failed:", error);
          }
        }
      }
      return result;
