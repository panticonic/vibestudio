...
describe("mobile Metro native capability boundary", () => {
  const workspaceAppRoot = path.join(exactTemplateRoots.system, "apps/mobile");
  ...
  it("does not affect normal JavaScript package resolution", () => {
    expect(() =>
      boundary.guardNativeModuleImport("@vibestudio/shared", path.join(workspaceAppRoot, "App.tsx"))
    ).not.toThrow();
  });
