...
function lockDir(): string {
  return process.env["VIBESTUDIO_PORT_LOCK_DIR"] ?? path.join(os.tmpdir(), "vibestudio-port-locks");
}
...
function removeStaleLock(filePath: string): void {
  ...
  try {
    ...
    // Another process may have won stale-lock cleanup.
  }
}
...
export function releaseServicePort(service: keyof typeof PORT_RANGES, port: number): void {
  const key = leaseKey(service, port);
  ...
  try {
    fs.unlinkSync(lockPath(service, port));
  } catch {
