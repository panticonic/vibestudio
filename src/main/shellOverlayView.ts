...
export class ShellOverlayView {
  ...
  constructor(
    ...
    ipcMain.on("vibestudio:shell-overlay:event", this.handleOverlayEvent);
    ipcMain.on("vibestudio:shell-overlay:hide", this.handleHide);
  }
  ...
  destroy(): void {
    ipcMain.removeListener("vibestudio:shell-overlay:event", this.handleOverlayEvent);
    ipcMain.removeListener("vibestudio:shell-overlay:hide", this.handleHide);
    if (this.view && !this.view.webContents.isDestroyed()) {
  ...
  private flushPayload(view: WebContentsView): void {
    ...
    view.webContents.send("vibestudio:shell-overlay:render", this.pendingPayload);
    this.pendingPayload = null;
  }
