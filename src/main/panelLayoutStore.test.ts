...
describe("PanelLayoutStore", () => {
  ...
  it("round-trips an opaque blob across a reload, keyed per workspace and account", async () => {
    const dir = tempDir();
  ...
  it("tolerates a corrupt file by returning null", async () => {
    const dir = tempDir();
    const store = new PanelLayoutStore(dir);
  ...
  it("encodes path-hostile ids so keys cannot escape the store directory", async () => {
    const dir = tempDir();
    const store = new PanelLayoutStore(dir);
  ...
  it("coalesces rapid writes and persists the newest layout", async () => {
    const dir = tempDir();
    const store = new PanelLayoutStore(dir);
    for (let version = 0; version < 20; version++) {
      store.set("ws-1", "user-1", { version });
    }
    await store.flush();
    expect(new PanelLayoutStore(dir).get("ws-1", "user-1")).toEqual({ version: 19 });
  });
});
