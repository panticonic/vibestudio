...
export class ViewManager {
  ...
  constructor(options: {
    ...
    this.nativeShellOverlay = new ShellOverlayView(
      ...
      (event) => {
        ...
        this.getShellChromeWebContents()?.send("vibestudio:shell-overlay:event", event);
      }
    );
    ...
    this.compositorRecovery = new CompositorRecovery(
      {
        ...
        logVerbose: (message) => log.trace(` ${message}`),
  ...
  setHostedShellReady(ownerViewId: string, ready: boolean, rendererInstanceId?: string): void {
    ...
    if (
      ...
      log.trace(` Ignore stale hosted shell cleanup from ${rendererInstanceId}`);
      return;
    }
  ...
  private ensureSlotLayerOrder(): boolean {
    ...
    for (const slot of this.nativePanelSlots.activeSlots.values()) {
      ...
      if (panelIndex !== -1 && panelIndex < shellIndex) {
        log.trace(` Slotted panel ${slot.panelId} is layered below the hosted shell — restacking`);
        this.reconcileNativeLayerOrder();
        return true;
      }
    }
