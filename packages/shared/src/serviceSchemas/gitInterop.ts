...
export const gitCreateDisposableRemoteOptionsSchema = z
  .object({
    name: z.string().optional().describe("Short display/repository name; defaults to workspace-test."),
...
export const gitDisposableRemoteInspectionSchema = z
  ...
  .strict();
export type GitDisposableRemoteInspection = z.infer<
  typeof gitDisposableRemoteInspectionSchema
>;
...
export type GitPushDisposableRemoteResult = z.infer<
  typeof gitPushDisposableRemoteResultSchema
>;
...
export const gitInteropProviderMethods = defineServiceMethods({
  ...
  pushDisposableRemote: {
    ...
    args: z.tuple([
      z
        .object({ repoPath: z.string(), url: z.string(), branch: z.string() })
        .strict(),
    ]),
    returns: z
      .object({
