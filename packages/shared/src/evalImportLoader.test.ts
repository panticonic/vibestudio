...
describe("createEvalImportLoader", () => {
  ...
  it("probes manifest-declared unscoped workspace units without building them", async () => {
    const call = vi.fn(async (_service: string, method: string) => {
      if (method === "inspectBuildProvenance") {
        return {
          ...
          unit: { name: "sample-do", kind: "worker", relativePath: "workers/sample-do" },
        };
      }
    ...
    await expect(loadImport.resolveWorkspaceImport("sample-do")).resolves.toBe(true);
    expect(call).toHaveBeenCalledWith("build", "inspectBuildProvenance", ["sample-do"]);
  });
