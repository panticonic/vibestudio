/**
 * Host-side WebSocket RPC client for owned launchers and the CLI.
 *
 ...
 * push (`channel tail`, `logs --follow`) and by owned external-agent brokers.
 *
...
export class WsRpcClient {
  ...
  private async connect(): Promise<{
    ...
    const clientLabel = this.config.clientLabel ?? "Vibestudio host client";
    const transport = wsClientTransport({
      selfId: this.config.callerId,
      getWsUrl: () => serverRpcWsUrl(this.config.url),
      reconnect: true,
      logPrefix: this.config.logPrefix ?? "[host-ws]",
