...
describe("approval markdown", () => {
  it("parses safe approval body formatting", () => {
    expect(
      parseApprovalMarkdown(
        "Run `sudo systemctl restart vibestudio`.\n\n- **Gate:** plugin\n- *Reason:* deploy update"
      )
    ).toEqual([
      {
        kind: "paragraph",
        children: [
          { kind: "text", text: "Run " },
          { kind: "code", text: "sudo systemctl restart vibestudio" },
          { kind: "text", text: "." },
        ],
      },
      {
  ...
  it("does not interpret HTML or links as active content", () => {
    expect(parseApprovalMarkdown("<script>alert(1)</script> [open](https://example.test)")).toEqual(
      [
        {
          kind: "paragraph",
          children: [
