...
describe("checkCapability", () => {
  ...
  it("matches '*' against a single path segment", () => {
    ...
    expect(checkCapability("https://api.example.com/v1/users/123", "GET", declarations)).toBe(
      "allow"
    );
    expect(
      checkCapability("https://api.example.com/v1/users/123/profile", "GET", declarations)
    ).toBe("deny");
  });
  ...
  it("matches '**' across multiple path segments", () => {
    ...
    expect(
      checkCapability(
        "https://api.example.com/v1/files/folder/nested/file.txt",
        "GET",
        declarations
      )
    ).toBe("allow");
  ...
  it("accepts '*' as a method wildcard", () => {
    ...
    expect(checkCapability("https://api.example.com/v1/users", "DELETE", declarations)).toBe(
      "allow"
    );
  });
  ...
  it("returns warn in warn mode when access would otherwise be denied", () => {
    ...
    expect(checkCapability("https://api.example.com/v1/users", "POST", declarations, "warn")).toBe(
      "warn"
    );
  });
});
