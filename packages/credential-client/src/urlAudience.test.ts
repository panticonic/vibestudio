...
describe("URL credential audiences", () => {
  ...
  it("keeps exact query strings exact and ordered", () => {
    ...
    expect(
      urlMatchesAudience("https://api.example.com/search", {
        url: "https://api.example.com/search",
        match: "exact",
      })
    ).toBe(true);
    expect(
      urlMatchesAudience("https://api.example.com/search?x=", {
        url: "https://api.example.com/search",
        match: "exact",
      })
    ).toBe(false);
  ...
  it("rejects unsafe audience URLs", () => {
    ...
    expect(() =>
      normalizeUrlAudience({ url: "https://user:pass@example.com/", match: "origin" })
    ).toThrow(/username/);
    expect(() => normalizeUrlAudience({ url: "http://192.168.1.3/", match: "origin" })).toThrow(
      /HTTPS/
    );
    expect(() => normalizeUrlAudience({ url: "https://example.com./", match: "origin" })).toThrow(
      /trailing-dot/
    );
  });
  ...
  it("allows localhost and loopback HTTP for local development", () => {
    expect(
      normalizeUrlAudience({ url: "http://localhost:3000/callback", match: "origin" }).url
    ).toBe("http://localhost:3000/");
    expect(
      normalizeUrlAudience({ url: "http://127.0.0.1:3000/callback", match: "path-prefix" }).url
    ).toBe("http://127.0.0.1:3000/callback");
  });

  it("finds the first matching audience", () => {
    expect(
      findMatchingUrlAudience("https://api.example.com/v1/models", [
...
describe("URL credential injection validation", () => {
  ...
  it("renders and strips credential carriers safely", () => {
    ...
    ).toEqual(["authorization", "proxy-authorization", "x-api-key", "x-custom-key"]);
  });

  it("rejects unsafe query parameter names", () => {
    expect(() =>
      normalizeCredentialInjection({ type: "query-param", name: "api_key" })
    ).not.toThrow();
    expect(() => normalizeCredentialInjection({ type: "query-param", name: "api&key" })).toThrow(
      /Invalid/
    );
    expect(() => normalizeCredentialInjection({ type: "query-param", name: "../token" })).toThrow(
      /Invalid/
    );
  });
});
