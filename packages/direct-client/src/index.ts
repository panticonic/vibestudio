...
export type StoredPairing = Omit<ConnectPairing, "code"> & { code?: string };

/** Transport/authentication failure independent of any CLI exit-code policy. */
export class DirectClientAuthError extends Error {
  readonly kind = "auth";
  constructor(message: string) {
    super(message);
    this.name = "DirectClientAuthError";
  }
}
...
export interface ExpectedHostIdentity {
  serverId: string;
  workspaceId?: string;
}

export interface DirectClientPolicy {
  /** Reject a listener/route that no longer resolves to the paired host. */
  expectedHost?: ExpectedHostIdentity;
  clientLabel?: string;
}
...
/** Preserve useful nested undici/system details without a CLI output dependency. */
...
async function fetchOrAuthError(url: URL, init: RequestInit): Promise<Response> {
  try {
    ...
    throw new DirectClientAuthError(`cannot reach ${url.origin}: ${networkErrorMessage(error)}`);
  }
}
...
export async function refreshShell(
  ...
  if (!parsed.success) {
    throw new DirectClientAuthError("shell refresh returned a malformed response");
  }
  return parsed.data;
}
...
export async function refreshAgent(creds: {
  ...
  if (!parsed.success) {
    throw new DirectClientAuthError("agent token exchange returned a malformed response");
  }
  return parsed.data;
}
...
export class RpcClient {
  ...
  private readonly pairing: StoredPairing | undefined;
  ...
  constructor(
    ...
    private readonly policy: DirectClientPolicy = {}
  ) {
    this.url = creds.url;
  ...
  private get isWebRtc(): boolean {
    return Boolean(this.pairing?.room && this.pairing.sig && this.pairing.fp);
  }
  ...
  private async refreshBearerToken(): Promise<string> {
    if (this.rawToken) {
      ...
      this.assertExpectedHost(refresh);
  ...
  private ensureWebRtcClient(): Promise<
  ...
  > {
    ...
    if (!this.webRtcClient) {
      ...
      this.webRtcClient = import("@vibestudio/direct-client/webrtc")
        .then(({ WebRtcRpcClient }) => {
          const client = new WebRtcRpcClient({
            ...
            clientLabel: this.policy.clientLabel ?? "Vibestudio direct client",
            onPaired: () => undefined,
          });
          return this.verifyPersistentHost(client);
        })
        .catch((error) => {
          this.webRtcClient = null;
          throw error;
  ...
  >(client: T): Promise<T> {
    ...
    this.assertExpectedHost(identity);
    return client;
  }

  private assertExpectedHost(identity: { serverId?: unknown; workspaceId?: unknown }): void {
    const expected = this.policy.expectedHost;
    if (!expected) return;
    if (identity.serverId !== expected.serverId) {
      throw new DirectClientAuthError(
        `Connected host identity changed: expected ${expected.serverId}, received ${String(identity.serverId)}`
      );
    }
    if (expected.workspaceId !== undefined && identity.workspaceId !== expected.workspaceId) {
      throw new DirectClientAuthError(
        `Connected workspace identity changed: expected ${expected.workspaceId}, received ${String(identity.workspaceId)}`
      );
    }
  }
}
...
function toRpcError(error: unknown): Error {
  if (error instanceof RpcError || error instanceof DirectClientAuthError) return error;
