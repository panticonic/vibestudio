...
describe("website chrome connection state", () => {
  it("displays revocation immediately and discards an older connected snapshot", async () => {
    ...
    const stop = observeWebsiteConnections({ list,
      listen: listener => { notify = listener; return vi.fn(); },
      subscribe: async () => {}, unsubscribe: async () => {}, changed, error: vi.fn(),
    });
    await flush();
    const entry = { slotId: "browser-slot", documentId: "fresh-document", connected: false };
    notify(entry);
  ...
  it("retires a late subscription without reading or publishing into a new workspace", async () => {
    ...
    const changed = vi.fn(); const list = vi.fn(); const unlisten = vi.fn();
    const stop = observeWebsiteConnections({ list, listen: () => unlisten,
      subscribe: () => subscribed.promise, unsubscribe, changed, error: vi.fn(),
    });
    stop();
