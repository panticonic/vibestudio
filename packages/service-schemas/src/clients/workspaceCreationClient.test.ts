...
function fixture() {
  ...
  const receipt = {
    operationId: "stable-operation-0001",
  ...
  const persistence = {
    ...
    newOperationId: vi.fn(() => receipt.operationId),
  };
  const client = {
    createWorkspace: vi.fn(async () => receipt),
    workspaceCreationReceipt: vi.fn(async () => receipt),
  };
...
describe("durable workspace creation submission", () => {
  it("persists before effects, and uses a fresh receipt read after a lost reply", async () => {
    ...
    await expect(
      submitWorkspaceCreation(f.client, { workspace: "Example" }, f.persistence)
    ...
    expect(f.client.workspaceCreationReceipt).toHaveBeenCalledWith({
      operationId: f.receipt.operationId,
    });
  ...
  it("keeps the original key and performs no new effect when reconciliation is denied", async () => {
    ...
    await expect(
      submitWorkspaceCreation(f.client, { workspace: "Example" }, f.persistence)
    ).rejects.toThrow("Receipt permission denied");
    expect(f.saved.size).toBe(1);
    expect(f.client.createWorkspace).toHaveBeenCalledOnce();
    expect(f.persistence.newOperationId).toHaveBeenCalledOnce();
  });
  ...
  it("does not replace unresolved input or silently recreate a deleted result", async () => {
    ...
    await expect(
      submitWorkspaceCreation(f.client, { workspace: "Different" }, f.persistence)
    ).rejects.toThrow("still unresolved");
    await expect(
      submitWorkspaceCreation(f.client, { workspace: "Example", purpose: "author" }, f.persistence)
    ).rejects.toThrow("still unresolved");
    const deleted = { ...f.receipt, state: "deleted" as const };
    const client = { ...f.client, workspaceCreationReceipt: vi.fn(async () => deleted) };
    await expect(
      submitWorkspaceCreation(client, { workspace: "Example" }, f.persistence)
