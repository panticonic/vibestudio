...
describe("PanelHandle", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@vibestudio/cdp-client");
  ...
  it("exposes panelTree roots/list/get and self handles", async () => {
    ...
    await expect(roots[0]?.observe()).resolves.toMatchObject({
      phase: "ready",
    });
  ...
  it("exposes a unified panel diagnostics bundle", async () => {
    ...
    expect(rpcCall).toHaveBeenCalledWith("main", "panelTree.diagnose", ["panel-1"]);
  });
  ...
  it("supports handle.click as a CDP automation convenience", async () => {
    ...
    (globalThis as any).__vibestudioRequireAsync__ = vi.fn(async (id: string) => {
      if (id === "@vibestudio/cdp-client") return { BrowserImpl: { connect } };
