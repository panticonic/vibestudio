...
import type { Browser, CdpPage } from "@vibestudio/cdp-client";
import type {
  CdpAutomation,
  CdpEndpoint,
...
const CDP_CLIENT_MODULE = "@vibestudio/cdp-client";
...
async function loadCdpClient(
  ...
  throw new Error(
    `Unable to load ${CDP_CLIENT_MODULE} for CDP automation. ` +
      `Call handle.cdp.page() only from contexts that expose @vibestudio/cdp-client.` +
