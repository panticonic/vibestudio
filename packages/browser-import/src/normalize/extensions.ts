...
export function parseChromiumManifest(
  ...
  return {
    ...
    homepageUrl: manifest["homepage_url"] ? String(manifest["homepage_url"]) : undefined,
    enabled,
  };
}
 ...
 */
export function parseFirefoxExtension(entry: Record<string, unknown>): ImportedExtension {
  return {
    id: String(entry["id"] || ""),
    name: String(
      entry["name"] || (entry["defaultLocale"] as Record<string, unknown>)?.["name"] || "Unknown"
    ),
    ...
    description: entry["description"]
      ...
      : (entry["defaultLocale"] as Record<string, unknown>)?.["description"]
        ...
        : undefined,
    homepageUrl: entry["homepageURL"] ? String(entry["homepageURL"]) : undefined,
